	...
	wikiServer.start({
		...
		function(err, handle) {
			if(err || !handle) {
				...
				$tw.desktop.utils.wiki.alert("Could not open this wiki: its local server failed to start. " +
					((err && err.message) || "")
				);
				// No window will ever exist for this entry — drop it from the window list, or
				// the wiki stays unopenable for the rest of the session. See handleOpenFailure.
				self.windowList.handleOpenFailure(self);
				return;
			}
			self.server = handle;
			$tw.desktop.gui.Window.open(handle.shellUrl, self.applyGeometryToOpenOptions({
...
WikiFileWindow.prototype.onloaded = function(event) {
	...
	try {
		var self = this;
		this.window_nwjs.on("new-win-policy", function(frame, url, policy) {
			...
			if(self.server && self.server.isShellUrl(url)) {
				console.warn("[TiddlyDesktop] refused to open a shell-path URL from the wiki:", url);
				policy.ignore();
				return;
			}
			if(url && /^file:\/\//i.test(url)) {
				policy.ignore(); // we open it ourselves below
				$tw.desktop.gui.Window.open(url, { show: true }, function(newWin) {
					newWin.once("loaded", function() {
						try {
							require("./utils/embeds.js").install(newWin.window.document,
								newWin.window
							);
							require("./utils/links.js").trapLinks(newWin.window.document);
						} catch(e) {
...
WikiFileWindow.prototype.installEmbedsOnTiddlerWindows = function() {
	...
	function tick() {
		...
		var pending = false;
		try {
			var iframeWin = self.iframe && self.iframe.contentWindow;
			...
			if(wins) {
				Object.keys(wins).forEach(function(id) {
					...
					require("./utils/embeds.js").install(w.document, w);
					require("./utils/links.js").trapLinks(w.document);
					// A tiddler popup is its own document, so it inherits nothing from the wiki's
					// <html> — and its editor is framed inside it in turn. Stamp both, once, as the
					// popup appears; a later settings change reaches it when it is next opened.
					spellcheck.applyToDocument(w.document, spellcheck.isEnabled($tw));
					spellcheck.observeFrames(w.document, function() {
...
// Apply the current local-spellcheck setting to the wiki's iframe document, and to any editor frames
// already open inside it. Safe to call any time.
...
WikiFileWindow.prototype.onloadiframe = function() {
	...
	// TiddlyWiki's text editor puts its <textarea> in an iframe of its own, created when the user
	// opens an editor — long after this load, and inheriting nothing from the wiki document. Stamp
	// each one as it appears, for as long as this document lives.
	self._iframeTeardowns.push(spellcheck.observeFrames(self.iframe.contentDocument, function() {
		return spellcheck.isEnabled($tw);
	})
	);
	...
	try {
		var backdrop = require("./utils/shell-backdrop.js").install({
			iframe: self.iframe,
			hostDocument: self.window_nwjs.window.document
		});
		if(backdrop) {
			self._iframeTeardowns.push(backdrop.teardown);
		}
	} catch(e) {
		console.error("[TiddlyDesktop] shell backdrop install failed:", e);
	}
	...
	$tw.desktop.utils.dragdrop.installImportInterceptor(this.iframe.contentDocument,
	...
	try {
		require("./utils/zoom.js").install(this.window_nwjs, this.window_nwjs.window.document,
	...
	try {
		...
		try {
			require("./utils/attachment-import.js").install(this.iframe.contentWindow,
				pathMod.dirname(this.pathname)
			);
		} catch(e) {
			console.error("[TiddlyDesktop] attachment import hook install failed:", e);
		}
		this._attachments = require("./utils/attachments.js").install(this.iframe.contentDocument,
			this.iframe.contentWindow,
			this.server,
			{
				wikiDir: pathMod.dirname(this.pathname),
	...
	try {
		var _tuSelf = this;
		var _trustUi = require("./utils/trust-ui.js").install(this.iframe.contentDocument,
			this.iframe.contentWindow,
				{
					identifier: this.getIdentifier(),
					wikiDir: pathMod.dirname(this.pathname),
					...
					openPicker: function(kind, seedPath, cb) {
						var hostDoc = _tuSelf.window_nwjs.window.document;
						...
						if(seedPath) {
							input.setAttribute("nwworkingdir", String(seedPath));
						}
	...
	try {
		var iframeTw = self.iframe.contentWindow && self.iframe.contentWindow.$tw;
		if(iframeTw && iframeTw.eventBus && iframeTw.eventBus.on) {
			...
			self._iframeTeardowns.push(function() {
				try {
					iframeTw.eventBus.off("window:opened", onPopupOpened);
				} catch(e) {}
			});
		}
	} catch(e) {
		console.error("[TiddlyDesktop] tiddler-window watch failed:", e);
	}
...
WikiFileWindow.prototype.onclose = function(event) {
	...
	//
	// Everything here has to tolerate a window that never finished loading: if the shell page or
	// the wiki iframe failed, this.iframe and the observers below were never created, and an
	// unguarded dereference throws INSIDE the close handler — which leaves the window on screen and
	// still in the window list, i.e. an app that cannot be closed. Folder windows already guard
	// theirs; this brings the two into line.
	var contentWindow = this.iframe && this.iframe.contentWindow;
	var onbeforeunload = contentWindow && contentWindow.onbeforeunload;
	if(onbeforeunload) {
