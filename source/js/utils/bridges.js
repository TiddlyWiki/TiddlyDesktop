...
exports.install = function(host) {
	...
	The whole queue is drained on each tick, not one item per tick. The WebSocket and LAN bridges
	below always did (`while (cmds.length)`); this one and the file bridge took a single item, so a
	burst of N requests cost N ticks — 200ms each — and the collab plugin's OAuth poll plus its
	relay REST calls queued behind one another for seconds. The requests themselves are async, so
	draining the queue starts them all and nothing is serialised by doing so.
	*/
		function _startHttpRequest(cw, item) {
			// Web schemes only. This bridge answers with the raw response body, so it
			// is a CORS-free fetch — a real capability the wiki page does not
			...
			var mod = item.url.substr(0, 8) === "https://" ? _httpsM : _httpM;
			mod.get(item.url, { headers: item.headers || {} }, function(res) {
				...
				var results = cw._nwjsHttpResults;
				if(results) {
					results[item.id] = { err: e.message || String(e) };
				}
			});
		}
		var _queueTimer = setInterval(function() {
			try {
				...
				var queue = cw && cw._nwjsHttpQueue;
				while(queue && queue.length) {
					_startHttpRequest(cw, queue.shift());
				}
			} catch(_e) {}
		}, 200);
		...
		host.iframe.contentWindow._nwjsOpenExternal = function(url) {
			...
			if(!/^https?:\/\//i.test(String(url))) {
				console.warn("[TiddlyDesktop] refusing to open a non-web URL:", url);
				return;
			}
		...
		if(typeof host.iframe.contentWindow._nwjsHttpQueueReady === "function") {
		...
		try {
			host.iframe.contentWindow._nwjsHostname = require("os").hostname();
		} catch(_e) {}
		...
		var _denyFileOp = function(cw, id, abs) {
			console.warn("[TiddlyDesktop] file bridge refused a path outside the wiki folder:", abs);
		...
		host.iframe.contentWindow._nwjsChooseSavePath = function(suggestedName, cb) {
		...
		/*
	One asset op per queued command, whole queue per tick — the same drain contract as the HTTP,
	WebSocket and LAN bridges. It used to take a single command per 100ms tick, which made a collab
	attachment (read in chunks) crawl in proportion to its size for no reason: the reads and writes
	are async, so starting them all costs nothing.
	*/
		function _runFileCommand(cw, item) {
			if(item.op === "read") {
				...
				_fsMod.readFile(src, function(err, buf) {
					var r = cw._nwjsFileResults;
					if(!r) return;
					r[item.id] = err
					? { err: err.message }
				...
				try {
					_fsMod.mkdirSync(_pathMod.dirname(dest), { recursive: true });
				} catch(_e) {}
				...
				// An unknown op must still answer: the iframe polls _nwjsFileResults for this id and
				// would otherwise wait for a result that is never written.
				var r = cw._nwjsFileResults;
				if(r) {
					r[item.id] = { err: "unsupported file op: " + item.op };
				}
			}
		}
			...
			try {
				...
				while(cmds && cmds.length) {
					...
					if(cmd.op === "create") {
						...
						(function(id, url, hdrs) {
							var wsHeaders = {
								"User-Agent": "TiddlyDesktop/1.0 NW.js"
							};
							...
							try {
								...
								sock.on("message", function(d, meta) {
									if(cw._nwjsWsEventQueue)
									cw._nwjsWsEventQueue.push({
										id: id,
										type: "message",
										data: meta && meta.binary
										? d
										: d.toString("utf8")
									}
									);
								...
								sock.on("error", function(e) {
									console.error("[ws-bridge] Error id=" + id + ":", e && e.message);
									if(cw._nwjsWsEventQueue)
									cw._nwjsWsEventQueue.push({
										id: id,
										type: "error",
				...
				if(evts && evts.length && typeof cw._nwjsWsOnEvent === "function") {
					while(evts.length) {
						var ev = evts.shift();
						try {
							cw._nwjsWsOnEvent(ev.id, ev.type, ev.data);
						} catch(_e) {}
					}
				}
			} catch(_e) {}
		...
		host.iframe.contentWindow._nwjsWsCreate = function(url, headers) {
			var id = ++_wsIdSeq;
		...
		if(typeof host.iframe.contentWindow._nwjsWsBridgeReady === "function") {
			...
			try {
				...
				while(cmds && cmds.length) {
					...
					if(cmd.op === "init") {
						...
						try {
							_lanNode = require("./lan-node.js").createLanNode({
								deviceId: cmd.deviceId,
								roomKey: cmd.roomKeyHex
								? Buffer.from(cmd.roomKeyHex, "hex")
								: null,
								onReady: function(pub, eps) {
									if(cw._nwjsLanEventQueue)
							...
							console.error("[lan-bridge] init failed:", e && e.message);
						}
					} else if(cmd.op === "addpeer") {
						if(_lanNode) {
							_lanNode.addPeer(cmd.deviceId, cmd.pubKeyB64, cmd.endpoints);
						}
					} else if(cmd.op === "broadcast") {
						if(_lanNode) {
							_lanNode.broadcast(cmd.json);
						}
				...
				while(evts && evts.length) {
					var ev = evts.shift();
					try {
						if(ev.type === "ready" && typeof cw._nwjsLanOnReady === "function") {
							cw._nwjsLanOnReady(ev.pub, ev.eps);
						} else if(
							ev.type === "message" && typeof cw._nwjsLanOnMessage === "function"
						) {
							cw._nwjsLanOnMessage(ev.peerId, ev.json);
						} else if(
							ev.type === "peers" && typeof cw._nwjsLanOnPeers === "function"
						) {
							cw._nwjsLanOnPeers(ev.n);
						}
					} catch(_e) {}
				}
			} catch(_e) {}
		...
		host.iframe.contentWindow._nwjsLanInit = function(roomKeyHex, did) {
		...
		host.iframe.contentWindow._nwjsLanAddPeer = function(did, pubKeyB64, endpoints) {
			host.iframe.contentWindow._nwjsLanCmdQueue.push({
				op: "addpeer",
				deviceId: did,
				pubKeyB64: pubKeyB64,
		...
		if(typeof host.iframe.contentWindow._nwjsLanBridgeReady === "function") {
