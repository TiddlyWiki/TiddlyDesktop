...
var HL_ALL = "td-find-all",
	HL_CUR = "td-find-current";
...
exports.installFindBar = function (options) {
	...
	// ── form-control match overlays ─────────────────────────────────────────────
	// The CSS Custom Highlight API can't paint inside a <textarea>/<input>, and a control's
	// native (inactive) selection isn't rendered once focus moves to the find bar — least of
	// all for TiddlyWiki's *framed* editor, whose textarea lives in a nested iframe that's
	// fully blurred. So we draw our own translucent highlighter boxes over the matched text:
	// measure the run's rectangles with a hidden mirror element, project them up through any
	// nested iframes into the HOST window, and lay plain divs there. The host document is not
	// the one we observe for mutations, so these overlays can't feed back into the search.
	var fieldLayer = null,
		repositionRaf = null;
	...
	function ensureFieldLayer() {
		...
		fieldLayer.className = "td-find-field-layer";
		fieldLayer.setAttribute("aria-hidden", "true");
		var s = fieldLayer.style;
		s.position = "fixed";
		s.left = "0";
		s.top = "0";
		s.width = "0";
		...
		s.zIndex = "2147483645";
		hostDocument.body.appendChild(fieldLayer);
		return fieldLayer;
	}
	...
	// On-screen rectangles (in el's own document viewport) covering el.value[start..end],
	// accounting for line wrapping, padding/border and the control's internal scroll. A hidden
	// mirror div replicates the control's text layout so getClientRects on the matched run lands
	// where the real glyphs are; results are clipped to the control's visible content box.
	function measureFieldRects(el, start, end) {
		...
		// The mirror must wrap at exactly the same column as the control, so replicate its
		// CONTENT box. clientWidth is content + padding minus any scrollbar, so the content
		// width is clientWidth - horizontal padding; forcing box-sizing:content-box then keeps
		// this correct no matter the control's own box-sizing. (A border-box textarea with
		// padding otherwise wraps a few px early, drifting every wrapped line's overlay sideways.)
		var contentWidth = Math.max(0, el.clientWidth - padL - padR);
		var div = doc.createElement("div");
		div.className = "td-find-measure";
		var ds = div.style;
		ds.position = "absolute";
		ds.top = "0";
		ds.left = "-9999px";
		ds.visibility = "hidden";
		ds.whiteSpace = el.nodeName === "INPUT" ? "pre" : "pre-wrap";
		ds.overflowWrap = "break-word";
		ds.wordBreak = style.wordBreak;
		ds.margin = "0";
		...
		div.appendChild(doc.createTextNode(value.substring(0, start)));
		div.appendChild(span);
		div.appendChild(doc.createTextNode(value.substring(end)));
		doc.body.appendChild(div);
		var out = [];
		try {
			var taRect = el.getBoundingClientRect(),
				divRect = div.getBoundingClientRect();
			var sx = el.scrollLeft,
				sy = el.scrollTop;
			var bl = parseFloat(style.borderLeftWidth) || 0,
				bt = parseFloat(style.borderTopWidth) || 0,
				brw = parseFloat(style.borderRightWidth) || 0,
				bbw = parseFloat(style.borderBottomWidth) || 0;
			var clipL = taRect.left + bl,
				clipT = taRect.top + bt,
	...
	// Map a rect from `fromDoc`'s viewport up through each enclosing iframe to the host viewport,
	// so an overlay placed (fixed) in the host window sits exactly over the control — even when
	...
	function applyHighlights() {
		// Field matches are painted as our own overlay boxes (the Highlight API can't reach
	...
	function renderCount() {
		...
		prevBtn.disabled = noMatches;
		nextBtn.disabled = noMatches;
	}
	...
	// Keep field overlays glued to their controls while the user scrolls/resizes. Scroll is
	// captured (it doesn't bubble) so inner scrollers — the story river, a scrollable editor —
	// are caught too. Tracked so we can detach exactly what we attached on close.
	var repoTargets = [];
