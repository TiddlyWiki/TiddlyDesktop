...
function getBundledLanguages() {
	try {
		...
		return fs.readdirSync(langDir).filter(function(name) {
			try {
				return fs.statSync(path.resolve(langDir, name)).isDirectory();
			} catch(e) {
				return false;
			}
		}).sort();
...
exports.getBackstageWikiFolder = function(appDataPath) {
	...
	if(fs.existsSync(wikiFolder) && fs.existsSync(packageFilename)) {
		...
		packageJson = {
			...
			plugins: ["tiddlywiki/filesystem", "tiddlywiki/tiddlydesktop"],
	...
	var defaultThemes = ["tiddlywiki/vanilla", "tiddlywiki/snowwhite"];
