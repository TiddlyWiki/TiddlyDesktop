...
var KEY_ORDER = [
	...
	"Share/TemplatesHeading","Share/TemplatesHelp","Share/GeneratedHelp","Share/DeleteTemplate",
	"Share/TitleLabel","Share/TagsLabel",
	...
	"Toolbar/Support","Support/Blurb","Support/Button",
	"PluginChooser/BackstageTarget","PluginChooser/BackstageNote","PluginChooser/SavedPendingRestart",
	"TrustedPaths/Revoke","Advanced/Unsandboxed","Advanced/UnsandboxedHelp",
	"Settings/Backups","Settings/Flags","Settings/Spellcheck","Settings/Plugins","Settings/Templates",
	"Settings/UserConfig","Settings/WikiListTitle","Settings/TrustedPaths","Settings/Version",
	"Settings/ResetToDefault","Settings/OpenChromiumFlags","Settings/OpenConfigFolder",
	"Settings/InstallPlugins",
	"Spellcheck/Enable","Spellcheck/LanguageHeading","Spellcheck/GoogleHeading",
	"Spellcheck/EnableGoogle","Spellcheck/DiagnosticsHeading","Spellcheck/Refresh",
	"TrustedPaths/RevokeTooltip",
	"Convert/NoDestination","Convert/ToFile","Convert/ToFolder","Convert/OriginalKept",
	"Convert/InProgress","Convert/Failed","Convert/Done"
];
...
var SHARE_SETTINGS = {
	"de-DE": ["Freigabe-Vorlagen","Wenn du einen Link in ein Wiki teilst, reichert TiddlyDesktop ihn an (Titel, Beschreibung, Bild, Einbettung) und erstellt aus der passenden Vorlage unten einen Tiddler. Platzhalter:","Diese Vorlage löschen","Schlagwörter:","Eigene Vorlage hinzufügen","Gib ihr einen Namen (um sie über eine Domänenregel unten anzusprechen) und bearbeite dann oben ihren Inhalt. Sie beginnt als Kopie der generischen Vorlage.","z. B. mastodon","Vorlage erstellen","Domänenregeln","Eine Regel pro Zeile als <code>domain=kind</code> (z. B. <code>vimeo.com=youtube</code> oder <code>mastodon.social=mastodon</code> für eine oben hinzugefügte Vorlage), um eine Website einer bestimmten Vorlage zuzuordnen.","vimeo.com=youtube","Sicherungen von Einzeldatei-Wikis werden standardmäßig neben jedem Wiki gespeichert. Wähle hier einen Ordner, um stattdessen alle Sicherungen an einem Ort zu sammeln.","Wird im Moment des Teilens erzeugt – vor allem für den Titel, damit zweimaliges Teilen derselben Seite nicht zwei Tiddler mit demselben Namen ergibt:","Titel:"],
	"fr-FR": ["Modèles de partage","Lorsque vous partagez un lien dans un wiki, TiddlyDesktop l'enrichit (titre, description, image, intégration) et crée un tiddler à partir du modèle correspondant ci-dessous. Espaces réservés :","Supprimer ce modèle","Étiquettes :","Ajouter votre propre modèle","Donnez-lui un nom (pour le cibler depuis une règle de domaine ci-dessous), puis modifiez son contenu ci-dessus. Il démarre comme une copie du modèle générique.","p. ex. mastodon","Créer le modèle","Règles de domaine","Une règle par ligne sous la forme <code>domain=kind</code> (p. ex. <code>vimeo.com=youtube</code>, ou <code>mastodon.social=mastodon</code> pour un modèle ajouté ci-dessus) pour associer un site à un modèle particulier.","vimeo.com=youtube","Les sauvegardes des wikis mono-fichier sont écrites à côté de chaque wiki par défaut. Choisissez un dossier ici pour regrouper toutes les sauvegardes au même endroit.","Généré au moment du partage — surtout pour le titre, afin que partager deux fois la même page ne produise pas deux tiddlers du même nom :","Titre :"],
	"es-ES": ["Plantillas de compartición","Cuando compartes un enlace en un wiki, TiddlyDesktop lo enriquece (título, descripción, imagen, incrustación) y crea un tiddler a partir de la plantilla coincidente de abajo. Marcadores:","Eliminar esta plantilla","Etiquetas:","Añade tu propia plantilla","Ponle un nombre (para apuntarla desde una regla de dominio de abajo) y luego edita su contenido arriba. Empieza como una copia de la plantilla genérica.","p. ej. mastodon","Crear plantilla","Reglas de dominio","Una regla por línea como <code>domain=kind</code> (p. ej. <code>vimeo.com=youtube</code>, o <code>mastodon.social=mastodon</code> para una plantilla que hayas añadido arriba) para dirigir un sitio a una plantilla concreta.","vimeo.com=youtube","Las copias de seguridad de los wikis de archivo único se guardan junto a cada wiki de forma predeterminada. Elige aquí una carpeta para reunir todas las copias en un solo lugar.","Se genera en el momento de compartir: sobre todo para el título, para que compartir dos veces la misma página no produzca dos tiddlers con el mismo nombre:","Título:"],
	"it-IT": ["Modelli di condivisione","Quando condividi un link in un wiki, TiddlyDesktop lo arricchisce (titolo, descrizione, immagine, incorporamento) e crea un tiddler dal modello corrispondente qui sotto. Segnaposti:","Elimina questo modello","Tag:","Aggiungi il tuo modello","Dagli un nome (per richiamarlo da una regola di dominio qui sotto), poi modifica il suo contenuto sopra. Parte come copia del modello generico.","es. mastodon","Crea modello","Regole di dominio","Una regola per riga come <code>domain=kind</code> (es. <code>vimeo.com=youtube</code>, oppure <code>mastodon.social=mastodon</code> per un modello aggiunto sopra) per indirizzare un sito a un modello specifico.","vimeo.com=youtube","I backup dei wiki a file singolo vengono salvati accanto a ciascun wiki per impostazione predefinita. Scegli qui una cartella per raccogliere tutti i backup in un unico posto.","Generato nel momento della condivisione: soprattutto per il titolo, così condividere due volte la stessa pagina non crea due tiddler con lo stesso nome:","Titolo:"],
	"pt-PT": ["Modelos de partilha","Quando partilha uma ligação num wiki, o TiddlyDesktop enriquece-a (título, descrição, imagem, incorporação) e cria um tiddler a partir do modelo correspondente abaixo. Marcadores:","Eliminar este modelo","Etiquetas:","Adicione o seu próprio modelo","Dê-lhe um nome (para o referenciar a partir de uma regra de domínio abaixo) e depois edite o seu conteúdo acima. Começa como uma cópia do modelo genérico.","p. ex. mastodon","Criar modelo","Regras de domínio","Uma regra por linha como <code>domain=kind</code> (p. ex. <code>vimeo.com=youtube</code>, ou <code>mastodon.social=mastodon</code> para um modelo que adicionou acima) para encaminhar um site para um modelo específico.","vimeo.com=youtube","As cópias de segurança de wikis de ficheiro único são guardadas junto a cada wiki por predefinição. Escolha aqui uma pasta para reunir todas as cópias num só local.","Gerado no momento em que partilha — sobretudo para o título, para que partilhar duas vezes a mesma página não produza dois tiddlers com o mesmo nome:","Título:"],
	"pt-BR": ["Modelos de compartilhamento","Quando você compartilha um link em um wiki, o TiddlyDesktop o enriquece (título, descrição, imagem, incorporação) e cria um tiddler a partir do modelo correspondente abaixo. Espaços reservados:","Excluir este modelo","Tags:","Adicione seu próprio modelo","Dê um nome a ele (para referenciá-lo a partir de uma regra de domínio abaixo) e edite seu conteúdo acima. Ele começa como uma cópia do modelo genérico.","ex. mastodon","Criar modelo","Regras de domínio","Uma regra por linha como <code>domain=kind</code> (ex. <code>vimeo.com=youtube</code>, ou <code>mastodon.social=mastodon</code> para um modelo que você adicionou acima) para direcionar um site para um modelo específico.","vimeo.com=youtube","Os backups de wikis de arquivo único são gravados ao lado de cada wiki por padrão. Escolha uma pasta aqui para reunir todos os backups em um só lugar.","Gerado no momento em que você compartilha — principalmente para o título, para que compartilhar a mesma página duas vezes não produza dois tiddlers com o mesmo nome:","Título:"],
	"nl-NL": ["Deelsjablonen","Wanneer je een link in een wiki deelt, verrijkt TiddlyDesktop deze (titel, beschrijving, afbeelding, insluiting) en bouwt een tiddler op basis van het overeenkomende sjabloon hieronder. Plaatsaanduidingen:","Dit sjabloon verwijderen","Labels:","Eigen sjabloon toevoegen","Geef het een naam (om het aan te sturen vanuit een domeinregel hieronder) en bewerk vervolgens de inhoud hierboven. Het begint als een kopie van het generieke sjabloon.","bijv. mastodon","Sjabloon maken","Domeinregels","Eén regel per regel als <code>domain=kind</code> (bijv. <code>vimeo.com=youtube</code>, of <code>mastodon.social=mastodon</code> voor een sjabloon dat je hierboven hebt toegevoegd) om een site naar een bepaald sjabloon te leiden.","vimeo.com=youtube","Back-ups van enkelbestandswiki's worden standaard naast elke wiki opgeslagen. Kies hier een map om in plaats daarvan alle back-ups op één plek te verzamelen.","Wordt gegenereerd op het moment van delen — vooral voor de titel, zodat dezelfde pagina twee keer delen geen twee tiddlers met dezelfde naam oplevert:","Titel:"],
	"pl-PL": ["Szablony udostępniania","Gdy udostępniasz link do wiki, TiddlyDesktop wzbogaca go (tytuł, opis, obraz, osadzenie) i tworzy tiddlera z pasującego szablonu poniżej. Symbole zastępcze:","Usuń ten szablon","Tagi:","Dodaj własny szablon","Nadaj mu nazwę (aby wskazać go z reguły domeny poniżej), a następnie edytuj jego treść powyżej. Zaczyna jako kopia szablonu ogólnego.","np. mastodon","Utwórz szablon","Reguły domen","Jedna reguła w wierszu jako <code>domain=kind</code> (np. <code>vimeo.com=youtube</code> lub <code>mastodon.social=mastodon</code> dla szablonu dodanego powyżej), aby kierować witrynę do konkretnego szablonu.","vimeo.com=youtube","Kopie zapasowe wiki jednoplikowych są domyślnie zapisywane obok każdej wiki. Wybierz tutaj folder, aby zamiast tego zebrać wszystkie kopie w jednym miejscu.","Generowane w chwili udostępniania — głównie dla tytułu, aby dwukrotne udostępnienie tej samej strony nie utworzyło dwóch tiddlerów o tej samej nazwie:","Tytuł:"],
	"ru-RU": ["Шаблоны публикации","Когда вы делитесь ссылкой в вики, TiddlyDesktop обогащает её (заголовок, описание, изображение, встраивание) и создаёт тиддлер из подходящего шаблона ниже. Заполнители:","Удалить этот шаблон","Метки:","Добавьте свой шаблон","Дайте ему имя (чтобы указать на него из правила домена ниже), затем отредактируйте его содержимое выше. Он начинается как копия универсального шаблона.","напр. mastodon","Создать шаблон","Правила доменов","Одно правило на строку в виде <code>domain=kind</code> (напр. <code>vimeo.com=youtube</code> или <code>mastodon.social=mastodon</code> для добавленного выше шаблона), чтобы направить сайт к определённому шаблону.","vimeo.com=youtube","Резервные копии одностраничных вики по умолчанию сохраняются рядом с каждой вики. Выберите здесь папку, чтобы вместо этого собрать все копии в одном месте.","Создаётся в момент публикации — прежде всего для заголовка, чтобы повторная публикация той же страницы не давала два тиддлера с одинаковым именем:","Заголовок:"],
	"sv-SE": ["Delningsmallar","När du delar en länk till en wiki berikar TiddlyDesktop den (titel, beskrivning, bild, inbäddning) och bygger en tiddler från den matchande mallen nedan. Platshållare:","Ta bort den här mallen","Taggar:","Lägg till din egen mall","Ge den ett namn (för att rikta in den från en domänregel nedan) och redigera sedan dess innehåll ovan. Den börjar som en kopia av den generiska mallen.","t.ex. mastodon","Skapa mall","Domänregler","En regel per rad som <code>domain=kind</code> (t.ex. <code>vimeo.com=youtube</code>, eller <code>mastodon.social=mastodon</code> för en mall du lagt till ovan) för att styra en webbplats till en viss mall.","vimeo.com=youtube","Säkerhetskopior av enfilswikis skrivs som standard bredvid varje wiki. Välj en mapp här för att i stället samla alla säkerhetskopior på ett ställe.","Genereras i det ögonblick du delar — främst för titeln, så att dela samma sida två gånger inte ger två tiddlers med samma namn:","Titel:"],
	"da-DK": ["Delingsskabeloner","Når du deler et link i en wiki, beriger TiddlyDesktop det (titel, beskrivelse, billede, indlejring) og bygger en tiddler ud fra den matchende skabelon nedenfor. Pladsholdere:","Slet denne skabelon","Mærker:","Tilføj din egen skabelon","Giv den et navn (for at målrette den fra en domæneregel nedenfor), og rediger derefter dens indhold ovenfor. Den starter som en kopi af den generiske skabelon.","f.eks. mastodon","Opret skabelon","Domæneregler","Én regel pr. linje som <code>domain=kind</code> (f.eks. <code>vimeo.com=youtube</code>, eller <code>mastodon.social=mastodon</code> for en skabelon, du har tilføjet ovenfor) for at dirigere et websted til en bestemt skabelon.","vimeo.com=youtube","Sikkerhedskopier af enkeltfil-wikier skrives som standard ved siden af hver wiki. Vælg en mappe her for i stedet at samle alle sikkerhedskopier ét sted.","Genereres i det øjeblik, du deler — mest til titlen, så deling af den samme side to gange ikke giver to tiddlere med samme navn:","Titel:"],
	"cs-CZ": ["Šablony sdílení","Když sdílíte odkaz do wiki, TiddlyDesktop jej obohatí (název, popis, obrázek, vložení) a vytvoří tiddler z odpovídající šablony níže. Zástupné symboly:","Smazat tuto šablonu","Štítky:","Přidejte vlastní šablonu","Pojmenujte ji (abyste na ni cílili z pravidla domény níže) a poté upravte její obsah výše. Začíná jako kopie obecné šablony.","např. mastodon","Vytvořit šablonu","Pravidla domén","Jedno pravidlo na řádek jako <code>domain=kind</code> (např. <code>vimeo.com=youtube</code> nebo <code>mastodon.social=mastodon</code> pro šablonu přidanou výše) pro nasměrování webu na konkrétní šablonu.","vimeo.com=youtube","Zálohy jednosouborových wiki se ve výchozím nastavení ukládají vedle každé wiki. Zde vyberte složku, abyste místo toho shromáždili všechny zálohy na jednom místě.","Generuje se v okamžiku sdílení — hlavně pro název, aby dvojí sdílení téže stránky nevytvořilo dva tiddlery se stejným názvem:","Název:"],
	"sk-SK": ["Šablóny zdieľania","Keď zdieľate odkaz do wiki, TiddlyDesktop ho obohatí (názov, popis, obrázok, vloženie) a vytvorí tiddler z príslušnej šablóny nižšie. Zástupné symboly:","Odstrániť túto šablónu","Značky:","Pridajte vlastnú šablónu","Dajte jej názov (aby ste na ňu cielili z pravidla domény nižšie) a potom upravte jej obsah vyššie. Začína ako kópia všeobecnej šablóny.","napr. mastodon","Vytvoriť šablónu","Pravidlá domén","Jedno pravidlo na riadok ako <code>domain=kind</code> (napr. <code>vimeo.com=youtube</code> alebo <code>mastodon.social=mastodon</code> pre šablónu pridanú vyššie) na nasmerovanie stránky na konkrétnu šablónu.","vimeo.com=youtube","Zálohy jednosúborových wiki sa predvolene ukladajú vedľa každej wiki. Tu vyberte priečinok, aby ste namiesto toho zhromaždili všetky zálohy na jednom mieste.","Generuje sa v okamihu zdieľania — hlavne pre názov, aby dvojité zdieľanie tej istej stránky nevytvorilo dva tiddlery s rovnakým názvom:","Názov:"],
	"sl-SI": ["Predloge za deljenje","Ko delite povezavo v wiki, jo TiddlyDesktop obogati (naslov, opis, sliko, vdelavo) in iz ustrezne predloge spodaj ustvari tiddler. Ograde:","Izbriši to predlogo","Oznake:","Dodajte svojo predlogo","Poimenujte jo (za sklicevanje iz pravila domene spodaj), nato pa uredite njeno vsebino zgoraj. Začne kot kopija splošne predloge.","npr. mastodon","Ustvari predlogo","Pravila domen","Eno pravilo na vrstico kot <code>domain=kind</code> (npr. <code>vimeo.com=youtube</code> ali <code>mastodon.social=mastodon</code> za predlogo, dodano zgoraj), da usmerite spletno mesto na določeno predlogo.","vimeo.com=youtube","Varnostne kopije enodatotečnih wikijev se privzeto shranjujejo poleg vsakega wikija. Tukaj izberite mapo, da namesto tega zberete vse varnostne kopije na enem mestu.","Ustvari se v trenutku deljenja — predvsem za naslov, da dvakratno deljenje iste strani ne ustvari dveh tiddlerjev z istim imenom:","Naslov:"],
	"el-GR": ["Πρότυπα κοινοποίησης","Όταν κοινοποιείτε έναν σύνδεσμο σε ένα wiki, το TiddlyDesktop τον εμπλουτίζει (τίτλος, περιγραφή, εικόνα, ενσωμάτωση) και δημιουργεί ένα tiddler από το αντίστοιχο πρότυπο παρακάτω. Σύμβολα κράτησης θέσης:","Διαγραφή αυτού του προτύπου","Ετικέτες:","Προσθέστε το δικό σας πρότυπο","Δώστε του ένα όνομα (για να το στοχεύσετε από έναν κανόνα τομέα παρακάτω) και έπειτα επεξεργαστείτε το περιεχόμενό του παραπάνω. Ξεκινά ως αντίγραφο του γενικού προτύπου.","π.χ. mastodon","Δημιουργία προτύπου","Κανόνες τομέα","Ένας κανόνας ανά γραμμή ως <code>domain=kind</code> (π.χ. <code>vimeo.com=youtube</code> ή <code>mastodon.social=mastodon</code> για ένα πρότυπο που προσθέσατε παραπάνω) για να δρομολογήσετε έναν ιστότοπο σε ένα συγκεκριμένο πρότυπο.","vimeo.com=youtube","Τα αντίγραφα ασφαλείας των wiki ενός αρχείου γράφονται από προεπιλογή δίπλα σε κάθε wiki. Επιλέξτε έναν φάκελο εδώ για να συγκεντρώσετε όλα τα αντίγραφα ασφαλείας σε ένα μέρος.","Δημιουργείται τη στιγμή της κοινοποίησης — κυρίως για τον τίτλο, ώστε η κοινοποίηση της ίδιας σελίδας δύο φορές να μη δημιουργεί δύο tiddlers με το ίδιο όνομα:","Τίτλος:"],
	"ca-ES": ["Plantilles de compartició","Quan comparteixes un enllaç en un wiki, TiddlyDesktop l'enriqueix (títol, descripció, imatge, incrustació) i crea un tiddler a partir de la plantilla coincident de sota. Marcadors de posició:","Elimina aquesta plantilla","Etiquetes:","Afegeix la teva pròpia plantilla","Posa-li un nom (per orientar-la des d'una regla de domini a sota) i després edita'n el contingut a dalt. Comença com una còpia de la plantilla genèrica.","p. ex. mastodon","Crea la plantilla","Regles de domini","Una regla per línia com <code>domain=kind</code> (p. ex. <code>vimeo.com=youtube</code>, o <code>mastodon.social=mastodon</code> per a una plantilla que hagis afegit a dalt) per dirigir un lloc a una plantilla concreta.","vimeo.com=youtube","Les còpies de seguretat dels wikis d'un sol fitxer s'escriuen al costat de cada wiki de manera predeterminada. Tria una carpeta aquí per recollir totes les còpies en un sol lloc.","Es genera en el moment de compartir: sobretot per al títol, perquè compartir dues vegades la mateixa pàgina no produeixi dos tiddlers amb el mateix nom:","Títol:"],
	"ar-PS": ["قوالب المشاركة","عند مشاركة رابط في ويكي، يقوم TiddlyDesktop بإثرائه (العنوان، الوصف، الصورة، التضمين) وينشئ تيدلر من القالب المطابق أدناه. العناصر النائبة:","حذف هذا القالب","الوسوم:","أضف قالبك الخاص","امنحه اسمًا (لاستهدافه من قاعدة نطاق أدناه)، ثم عدّل محتواه أعلاه. يبدأ كنسخة من القالب العام.","مثال: mastodon","إنشاء قالب","قواعد النطاق","قاعدة واحدة في كل سطر بالشكل <code>domain=kind</code> (مثل <code>vimeo.com=youtube</code>، أو <code>mastodon.social=mastodon</code> لقالب أضفته أعلاه) لتوجيه موقع إلى قالب معيّن.","vimeo.com=youtube","تُكتب النسخ الاحتياطية لويكيات الملف الواحد بجانب كل ويكي افتراضيًا. اختر مجلدًا هنا لتجميع كل النسخ الاحتياطية في مكان واحد بدلاً من ذلك.","يُنشأ لحظة المشاركة — خاصةً للعنوان، حتى لا تُنتج مشاركة الصفحة نفسها مرتين تدلرين بالاسم نفسه:","العنوان:"],
	"fa-IR": ["قالب‌های هم‌رسانی","وقتی پیوندی را در یک ویکی هم‌رسانی می‌کنید، TiddlyDesktop آن را غنی می‌کند (عنوان، توضیح، تصویر، جاسازی) و از قالب منطبق زیر یک تیدلر می‌سازد. جای‌گیرها:","حذف این قالب","برچسب‌ها:","قالب خود را اضافه کنید","به آن نامی بدهید (برای هدف‌گیری از یک قاعدهٔ دامنه در زیر)، سپس محتوای آن را در بالا ویرایش کنید. به‌صورت یک نسخه از قالب عمومی آغاز می‌شود.","مثلاً mastodon","ساخت قالب","قواعد دامنه","هر خط یک قاعده به شکل <code>domain=kind</code> (مثلاً <code>vimeo.com=youtube</code> یا <code>mastodon.social=mastodon</code> برای قالبی که در بالا افزوده‌اید) برای هدایت یک سایت به قالبی خاص.","vimeo.com=youtube","نسخه‌های پشتیبان ویکی‌های تک‌پرونده‌ای به‌طور پیش‌فرض کنار هر ویکی نوشته می‌شوند. اینجا پوشه‌ای انتخاب کنید تا به‌جای آن همهٔ نسخه‌های پشتیبان در یک مکان جمع شوند.","در لحظهٔ هم‌رسانی ساخته می‌شود — بیشتر برای عنوان، تا هم‌رسانی دوبارهٔ همان صفحه دو تیدلر هم‌نام نسازد:","عنوان:"],
	"he-IL": ["תבניות שיתוף","כאשר אתה משתף קישור לוויקי, TiddlyDesktop מעשיר אותו (כותרת, תיאור, תמונה, הטמעה) ובונה טידלר מהתבנית התואמת שלמטה. מצייני מיקום:","מחק תבנית זו","תגיות:","הוסף תבנית משלך","תן לה שם (כדי לכוון אליה מכלל דומיין למטה), ואז ערוך את תוכנה למעלה. היא מתחילה כעותק של התבנית הכללית.","לדוגמה mastodon","צור תבנית","כללי דומיין","כלל אחד בכל שורה בצורה <code>domain=kind</code> (לדוגמה <code>vimeo.com=youtube</code>, או <code>mastodon.social=mastodon</code> עבור תבנית שהוספת למעלה) כדי לנתב אתר לתבנית מסוימת.","vimeo.com=youtube","גיבויים של ויקי בקובץ יחיד נכתבים לצד כל ויקי כברירת מחדל. בחר כאן תיקייה כדי לאסוף את כל הגיבויים במקום אחד במקום זאת.","נוצר ברגע השיתוף — בעיקר עבור הכותרת, כדי ששיתוף אותו עמוד פעמיים לא ייצור שני טידלרים בעלי אותו שם:","כותרת:"],
	"hi-IN": ["साझा टेम्पलेट","जब आप किसी विकी में लिंक साझा करते हैं, तो TiddlyDesktop उसे समृद्ध करता है (शीर्षक, विवरण, छवि, एम्बेड) और नीचे मिलान वाले टेम्पलेट से एक टिडलर बनाता है। प्लेसहोल्डर:","यह टेम्पलेट हटाएँ","टैग:","अपना टेम्पलेट जोड़ें","इसे एक नाम दें (नीचे किसी डोमेन नियम से इसे लक्षित करने के लिए), फिर ऊपर इसकी सामग्री संपादित करें। यह सामान्य टेम्पलेट की प्रति के रूप में शुरू होता है।","जैसे mastodon","टेम्पलेट बनाएँ","डोमेन नियम","प्रति पंक्ति एक नियम <code>domain=kind</code> के रूप में (जैसे <code>vimeo.com=youtube</code>, या ऊपर जोड़े गए टेम्पलेट के लिए <code>mastodon.social=mastodon</code>) किसी साइट को किसी विशेष टेम्पलेट पर भेजने के लिए।","vimeo.com=youtube","एकल-फ़ाइल विकी के बैकअप डिफ़ॉल्ट रूप से प्रत्येक विकी के साथ लिखे जाते हैं। इसके बजाय सभी बैकअप एक स्थान पर एकत्र करने के लिए यहाँ एक फ़ोल्डर चुनें।","साझा करते समय उत्पन्न होता है — मुख्यतः शीर्षक के लिए, ताकि एक ही पृष्ठ को दो बार साझा करने पर एक ही नाम के दो टिड्लर न बनें:","शीर्षक:"],
	"pa-IN": ["ਸਾਂਝੇ ਟੈਂਪਲੇਟ","ਜਦੋਂ ਤੁਸੀਂ ਕਿਸੇ ਵਿਕੀ ਵਿੱਚ ਲਿੰਕ ਸਾਂਝਾ ਕਰਦੇ ਹੋ, ਤਾਂ TiddlyDesktop ਇਸਨੂੰ ਭਰਪੂਰ ਕਰਦਾ ਹੈ (ਸਿਰਲੇਖ, ਵੇਰਵਾ, ਚਿੱਤਰ, ਏਮਬੈੱਡ) ਅਤੇ ਹੇਠਾਂ ਮੇਲ ਖਾਂਦੇ ਟੈਂਪਲੇਟ ਤੋਂ ਇੱਕ ਟਿਡਲਰ ਬਣਾਉਂਦਾ ਹੈ। ਪਲੇਸਹੋਲਡਰ:","ਇਹ ਟੈਂਪਲੇਟ ਮਿਟਾਓ","ਟੈਗ:","ਆਪਣਾ ਟੈਂਪਲੇਟ ਸ਼ਾਮਲ ਕਰੋ","ਇਸਨੂੰ ਇੱਕ ਨਾਮ ਦਿਓ (ਹੇਠਾਂ ਕਿਸੇ ਡੋਮੇਨ ਨਿਯਮ ਤੋਂ ਇਸਨੂੰ ਨਿਸ਼ਾਨਾ ਬਣਾਉਣ ਲਈ), ਫਿਰ ਉੱਪਰ ਇਸਦੀ ਸਮੱਗਰੀ ਸੰਪਾਦਿਤ ਕਰੋ। ਇਹ ਆਮ ਟੈਂਪਲੇਟ ਦੀ ਕਾਪੀ ਵਜੋਂ ਸ਼ੁਰੂ ਹੁੰਦਾ ਹੈ।","ਜਿਵੇਂ mastodon","ਟੈਂਪਲੇਟ ਬਣਾਓ","ਡੋਮੇਨ ਨਿਯਮ","ਪ੍ਰਤੀ ਲਾਈਨ ਇੱਕ ਨਿਯਮ <code>domain=kind</code> ਵਜੋਂ (ਜਿਵੇਂ <code>vimeo.com=youtube</code>, ਜਾਂ ਉੱਪਰ ਸ਼ਾਮਲ ਕੀਤੇ ਟੈਂਪਲੇਟ ਲਈ <code>mastodon.social=mastodon</code>) ਕਿਸੇ ਸਾਈਟ ਨੂੰ ਕਿਸੇ ਖਾਸ ਟੈਂਪਲੇਟ ਵੱਲ ਭੇਜਣ ਲਈ।","vimeo.com=youtube","ਇਕਹਿਰੀ-ਫਾਈਲ ਵਿਕੀ ਦੇ ਬੈਕਅੱਪ ਮੂਲ ਰੂਪ ਵਿੱਚ ਹਰ ਵਿਕੀ ਦੇ ਨਾਲ ਲਿਖੇ ਜਾਂਦੇ ਹਨ। ਇਸਦੀ ਬਜਾਏ ਸਾਰੇ ਬੈਕਅੱਪ ਇੱਕ ਥਾਂ ਇਕੱਠੇ ਕਰਨ ਲਈ ਇੱਥੇ ਇੱਕ ਫੋਲਡਰ ਚੁਣੋ।","ਸਾਂਝਾ ਕਰਨ ਵੇਲੇ ਬਣਾਇਆ ਜਾਂਦਾ ਹੈ — ਮੁੱਖ ਤੌਰ 'ਤੇ ਸਿਰਲੇਖ ਲਈ, ਤਾਂ ਜੋ ਇੱਕੋ ਸਫ਼ੇ ਨੂੰ ਦੋ ਵਾਰ ਸਾਂਝਾ ਕਰਨ 'ਤੇ ਇੱਕੋ ਨਾਂ ਦੇ ਦੋ ਟਿਡਲਰ ਨਾ ਬਣਨ:","ਸਿਰਲੇਖ:"],
	"ia-IA": ["Modellos de compartir","Quando tu comparti un ligamine in un wiki, TiddlyDesktop lo inricchi (titulo, description, imagine, incorporation) e construe un tiddler ex le modello concordante infra. Substitutos:","Deler iste modello","Etiquettas:","Adde tu proprie modello","Da lo un nomine (pro appellar lo ab un regula de dominio infra), pois redige su contento supra. Illo comencia como un copia del modello generic.","p.ex. mastodon","Crear modello","Regulas de dominio","Un regula per linea como <code>domain=kind</code> (p.ex. <code>vimeo.com=youtube</code>, o <code>mastodon.social=mastodon</code> pro un modello que tu addeva supra) pro diriger un sito a un modello specific.","vimeo.com=youtube","Le copias de reserva de wikis de file unic es scribite juxta cata wiki per predefecto. Selige un dossier hic pro colliger in loco tote le copias de reserva in un sol loco.","Generate al momento de compartir — principalmente pro le titulo, de sorta que compartir le mesme pagina duo vices non produce duo tiddlers con le mesme nomine:","Titulo:"],
	"ja-JP": ["共有テンプレート","ウィキにリンクを共有すると、TiddlyDesktop はそれを補強し（タイトル、説明、画像、埋め込み）、下の一致するテンプレートからティドラーを作成します。プレースホルダー:","このテンプレートを削除","タグ:","独自のテンプレートを追加","名前を付け（下のドメイン規則から指定するため）、上でその本文を編集します。汎用テンプレートのコピーとして始まります。","例: mastodon","テンプレートを作成","ドメイン規則","1 行に 1 つの規則を <code>domain=kind</code> の形式で（例: <code>vimeo.com=youtube</code>、または上で追加したテンプレート用に <code>mastodon.social=mastodon</code>）記述し、サイトを特定のテンプレートに振り分けます。","vimeo.com=youtube","単一ファイルウィキのバックアップは、既定では各ウィキの隣に書き込まれます。代わりにすべてのバックアップを 1 か所にまとめるには、ここでフォルダーを選択してください。","共有した時点で生成されます。主にタイトル用で、同じページを2回共有しても同名のティドラーが2つできないようにします:","タイトル:"],
	"ko-KR": ["공유 템플릿","위키에 링크를 공유하면 TiddlyDesktop이 이를 보강하고(제목, 설명, 이미지, 임베드) 아래의 일치하는 템플릿에서 티들러를 만듭니다. 자리 표시자:","이 템플릿 삭제","태그:","나만의 템플릿 추가","이름을 지정하고(아래 도메인 규칙에서 대상으로 지정하기 위해) 위에서 본문을 편집하세요. 일반 템플릿의 복사본으로 시작합니다.","예: mastodon","템플릿 만들기","도메인 규칙","사이트를 특정 템플릿으로 라우팅하려면 한 줄에 하나씩 <code>domain=kind</code> 형식으로 규칙을 작성하세요(예: <code>vimeo.com=youtube</code> 또는 위에서 추가한 템플릿의 경우 <code>mastodon.social=mastodon</code>).","vimeo.com=youtube","단일 파일 위키의 백업은 기본적으로 각 위키 옆에 기록됩니다. 대신 모든 백업을 한곳에 모으려면 여기에서 폴더를 선택하세요.","공유하는 시점에 생성됩니다. 주로 제목에 사용하여 같은 페이지를 두 번 공유해도 같은 이름의 티들러가 두 개 생기지 않도록 합니다:","제목:"],
	"mk-MK": ["Шаблони за споделување","Кога споделувате врска во вики, TiddlyDesktop ја збогатува (наслов, опис, слика, вградување) и создава tiddler од соодветниот шаблон подолу. Резервирани места:","Избриши го овој шаблон","Ознаки:","Додајте свој шаблон","Дајте му име (за да го насочите од правило за домен подолу), потоа уредете ја неговата содржина погоре. Започнува како копија на општиот шаблон.","на пр. mastodon","Создај шаблон","Правила за домени","Едно правило по ред како <code>domain=kind</code> (на пр. <code>vimeo.com=youtube</code>, или <code>mastodon.social=mastodon</code> за шаблон што го додадовте погоре) за да насочите сајт кон одреден шаблон.","vimeo.com=youtube","Резервните копии на вики со една датотека стандардно се запишуваат до секое вики. Изберете папка овде за наместо тоа да ги соберете сите резервни копии на едно место.","Се создава во моментот на споделување — главно за насловот, за да споделувањето на истата страница двапати не создаде два тидлера со исто име:","Наслов:"],
	"zh-Hans": ["分享模板","当你把链接分享到一个 wiki 时，TiddlyDesktop 会对其进行丰富（标题、描述、图片、嵌入），并根据下面匹配的模板生成一个 tiddler。占位符：","删除此模板","标签：","添加你自己的模板","给它取个名字（以便从下面的域名规则中引用它），然后在上方编辑其正文。它以通用模板的副本开始。","例如 mastodon","创建模板","域名规则","每行一条规则，格式为 <code>domain=kind</code>（例如 <code>vimeo.com=youtube</code>，或对于上面添加的模板使用 <code>mastodon.social=mastodon</code>），将某个站点路由到特定模板。","vimeo.com=youtube","单文件 wiki 的备份默认写入在每个 wiki 旁边。在此选择一个文件夹，以便将所有备份集中到一个位置。","在分享的那一刻生成——主要用于标题，这样同一个页面分享两次不会产生两个同名的 tiddler：","标题："],
	"zh-Hant": ["分享範本","當你將連結分享到一個 wiki 時，TiddlyDesktop 會加以豐富（標題、描述、圖片、嵌入），並根據下方相符的範本產生一個 tiddler。預留位置：","刪除此範本","標籤：","新增你自己的範本","為它命名（以便從下方的網域規則參照它），然後在上方編輯其內文。它以通用範本的副本開始。","例如 mastodon","建立範本","網域規則","每行一條規則，格式為 <code>domain=kind</code>（例如 <code>vimeo.com=youtube</code>，或對於上方新增的範本使用 <code>mastodon.social=mastodon</code>），將某個網站導向特定範本。","vimeo.com=youtube","單一檔案 wiki 的備份預設寫入在每個 wiki 旁邊。在此選擇一個資料夾，以便將所有備份集中到一個位置。","在分享的那一刻產生——主要用於標題，這樣同一個頁面分享兩次不會產生兩個同名的 tiddler：","標題："]
};
Object.keys(SHARE_SETTINGS).forEach(function(lang) {
	if(!T[lang]) { return; }
	var v = SHARE_SETTINGS[lang], keys = ["Share/TemplatesHeading","Share/TemplatesHelp","Share/DeleteTemplate","Share/TagsLabel","Share/AddHeading","Share/AddHelp","Share/NamePlaceholder","Share/CreateTemplate","Share/RulesHeading","Share/RulesHelp","Share/RulePlaceholder","Advanced/BackupsHelp","Share/GeneratedHelp","Share/TitleLabel"];
	keys.forEach(function(k, i) { T[lang][k] = v[i]; });
});

// ── strings added after the first translation pass ─────────────────────────
// Same shape as SHARE_SETTINGS above: one array per language, in LATER_KEYS order. These keys
// existed in plugins/tiddlydesktop/language/TiddlyDesktop.multids but had no translation, so the
// Support panel, the PluginChooser's backstage target, the trusted-paths revoke button and the
// unsandboxed escape hatch showed English in every non-English UI.
var LATER_KEYS = ["Toolbar/Support", "Support/Blurb", "Support/Button", "PluginChooser/BackstageTarget", "PluginChooser/BackstageNote", "PluginChooser/SavedPendingRestart", "TrustedPaths/Revoke", "Advanced/Unsandboxed", "Advanced/UnsandboxedHelp"];
var LATER_STRINGS = {
	"de-DE": ["Unterstützen", "TiddlyWiki und TiddlyDesktop sind kostenlos und quelloffen. Wenn du sie nützlich findest, unterstütze bitte ihre Weiterentwicklung auf Open Collective.", "Auf Open Collective unterstützen", "Diese Wiki-Liste", "wird nach einem Neustart von TiddlyDesktop wirksam", "Gespeichert. Starte TiddlyDesktop neu, um es zu aktivieren.", "widerrufen", "Dieses Wiki ohne Sandbox ausführen", "Das JavaScript dieses Wikis läuft dann mit vollem Zugriff auf deinen Computer — es kann jede Datei lesen, ändern und löschen, die du lesen kannst. Aktiviere das nur für ein Wiki, das du selbst geschrieben hast oder dem du vollständig vertraust, und nur wenn es Node-Module oder Shell-Befehle ausführen muss."],
	"fr-FR": ["Soutenir", "TiddlyWiki et TiddlyDesktop sont gratuits et open source. Si vous les trouvez utiles, envisagez de soutenir leur développement continu sur Open Collective.", "Soutenir sur Open Collective", "Cette liste de wikis", "prend effet après le redémarrage de TiddlyDesktop", "Enregistré. Redémarrez TiddlyDesktop pour l'activer.", "révoquer", "Exécuter ce wiki sans bac à sable", "Le JavaScript de ce wiki s'exécutera avec un accès complet à votre ordinateur — il pourra lire, modifier et supprimer tout fichier auquel vous avez accès. N'activez cette option que pour un wiki que vous avez écrit ou en lequel vous avez entièrement confiance, et seulement s'il doit exécuter des modules Node ou des commandes shell."],
	"es-ES": ["Apoyar", "TiddlyWiki y TiddlyDesktop son gratuitos y de código abierto. Si te resultan útiles, considera apoyar su desarrollo continuo en Open Collective.", "Apoyar en Open Collective", "Esta lista de wikis", "surte efecto tras reiniciar TiddlyDesktop", "Guardado. Reinicia TiddlyDesktop para activarlo.", "revocar", "Ejecutar este wiki sin el aislamiento (sandbox)", "El JavaScript de este wiki se ejecutará con acceso total a tu ordenador: podrá leer, modificar y eliminar cualquier archivo al que tú tengas acceso. Actívalo solo para un wiki que hayas escrito tú o en el que confíes plenamente, y solo si necesita ejecutar módulos de Node o comandos de shell."],
	"ca-ES": ["Donar suport", "TiddlyWiki i TiddlyDesktop són gratuïts i de codi obert. Si et resulten útils, considera donar suport al seu desenvolupament continu a Open Collective.", "Dona suport a Open Collective", "Aquesta llista de wikis", "té efecte després de reiniciar TiddlyDesktop", "Desat. Reinicia TiddlyDesktop per activar-ho.", "revoca", "Executa aquest wiki sense l'aïllament (sandbox)", "El JavaScript d'aquest wiki s'executarà amb accés total al teu ordinador: podrà llegir, modificar i eliminar qualsevol fitxer al qual tu tinguis accés. Activa-ho només per a un wiki que hagis escrit tu o en què confiïs plenament, i només si necessita executar mòduls de Node o ordres de shell."],
	"it-IT": ["Sostieni", "TiddlyWiki e TiddlyDesktop sono gratuiti e open source. Se li trovi utili, valuta di sostenere il loro sviluppo continuo su Open Collective.", "Sostieni su Open Collective", "Questo elenco di wiki", "ha effetto dopo il riavvio di TiddlyDesktop", "Salvato. Riavvia TiddlyDesktop per attivarlo.", "revoca", "Esegui questo wiki senza sandbox", "Il JavaScript di questo wiki verrà eseguito con accesso completo al tuo computer: potrà leggere, modificare ed eliminare qualsiasi file a cui tu abbia accesso. Attivalo solo per un wiki che hai scritto tu o di cui ti fidi completamente, e solo se deve eseguire moduli Node o comandi shell."],
	"pt-BR": ["Apoiar", "TiddlyWiki e TiddlyDesktop são gratuitos e de código aberto. Se você os achar úteis, considere apoiar seu desenvolvimento contínuo no Open Collective.", "Apoiar no Open Collective", "Esta lista de wikis", "entra em vigor após reiniciar o TiddlyDesktop", "Salvo. Reinicie o TiddlyDesktop para ativar.", "revogar", "Executar este wiki sem a sandbox", "O JavaScript deste wiki será executado com acesso total ao seu computador — ele poderá ler, alterar e excluir qualquer arquivo que você possa acessar. Ative isto apenas para um wiki que você mesmo escreveu ou em que confia plenamente, e somente se ele precisar executar módulos Node ou comandos de shell."],
	"pt-PT": ["Apoiar", "O TiddlyWiki e o TiddlyDesktop são gratuitos e de código aberto. Se os achar úteis, considere apoiar o seu desenvolvimento contínuo no Open Collective.", "Apoiar no Open Collective", "Esta lista de wikis", "entra em vigor após reiniciar o TiddlyDesktop", "Guardado. Reinicie o TiddlyDesktop para ativar.", "revogar", "Executar este wiki sem a sandbox", "O JavaScript deste wiki será executado com acesso total ao seu computador — poderá ler, alterar e eliminar qualquer ficheiro a que tenha acesso. Ative isto apenas para um wiki que tenha escrito ou em que confie plenamente, e só se precisar de executar módulos Node ou comandos de shell."],
	"nl-NL": ["Steunen", "TiddlyWiki en TiddlyDesktop zijn gratis en opensource. Als je ze nuttig vindt, overweeg dan hun voortdurende ontwikkeling te steunen op Open Collective.", "Steunen op Open Collective", "Deze wikilijst", "wordt van kracht na het herstarten van TiddlyDesktop", "Opgeslagen. Herstart TiddlyDesktop om het te activeren.", "intrekken", "Deze wiki zonder sandbox uitvoeren", "De eigen JavaScript van deze wiki draait dan met volledige toegang tot je computer — die kan elk bestand lezen, wijzigen en verwijderen dat jij kunt lezen. Schakel dit alleen in voor een wiki die je zelf hebt geschreven of volledig vertrouwt, en alleen als die Node-modules of shell-opdrachten moet uitvoeren."],
	"da-DK": ["Støt", "TiddlyWiki og TiddlyDesktop er gratis og open source. Hvis du finder dem nyttige, så overvej at støtte deres fortsatte udvikling på Open Collective.", "Støt på Open Collective", "Denne wikiliste", "træder i kraft efter genstart af TiddlyDesktop", "Gemt. Genstart TiddlyDesktop for at aktivere.", "tilbagekald", "Kør denne wiki uden sandkasse", "Denne wikis egen JavaScript kører med fuld adgang til din computer — den kan læse, ændre og slette enhver fil, du selv kan. Slå kun dette til for en wiki, du selv har skrevet eller stoler fuldt ud på, og kun hvis den skal køre node-moduler eller shell-kommandoer."],
	"sv-SE": ["Stöd", "TiddlyWiki och TiddlyDesktop är gratis och öppen källkod. Om du har nytta av dem, överväg att stödja deras fortsatta utveckling på Open Collective.", "Stöd på Open Collective", "Den här wikilistan", "träder i kraft efter omstart av TiddlyDesktop", "Sparat. Starta om TiddlyDesktop för att aktivera.", "återkalla", "Kör denna wiki utan sandlåda", "Den här wikins egen JavaScript körs med full åtkomst till din dator — den kan läsa, ändra och radera alla filer du själv kan. Slå bara på detta för en wiki du själv har skrivit eller litar fullständigt på, och bara om den behöver köra node-moduler eller skalkommandon."],
	"pl-PL": ["Wesprzyj", "TiddlyWiki i TiddlyDesktop są darmowe i mają otwarty kod. Jeśli uważasz je za przydatne, rozważ wsparcie ich dalszego rozwoju na Open Collective.", "Wesprzyj na Open Collective", "Ta lista wiki", "zacznie działać po ponownym uruchomieniu TiddlyDesktop", "Zapisano. Uruchom ponownie TiddlyDesktop, aby aktywować.", "cofnij", "Uruchom to wiki bez piaskownicy", "JavaScript tego wiki będzie działać z pełnym dostępem do Twojego komputera — może odczytać, zmienić i usunąć każdy plik, do którego Ty masz dostęp. Włącz to tylko dla wiki, które sam napisałeś lub któremu w pełni ufasz, i tylko jeśli musi uruchamiać moduły Node lub polecenia powłoki."],
	"cs-CZ": ["Podpořit", "TiddlyWiki a TiddlyDesktop jsou zdarma a s otevřeným kódem. Pokud je považujete za užitečné, zvažte podporu jejich dalšího vývoje na Open Collective.", "Podpořit na Open Collective", "Tento seznam wiki", "projeví se po restartu TiddlyDesktopu", "Uloženo. Pro aktivaci restartujte TiddlyDesktop.", "odvolat", "Spustit tuto wiki bez sandboxu", "JavaScript této wiki poběží s plným přístupem k vašemu počítači — může číst, měnit a mazat jakýkoli soubor, ke kterému máte přístup. Zapněte to jen pro wiki, kterou jste sami napsali nebo které plně důvěřujete, a jen pokud potřebuje spouštět moduly Node nebo příkazy shellu."],
	"sk-SK": ["Podporiť", "TiddlyWiki a TiddlyDesktop sú zadarmo a s otvoreným zdrojovým kódom. Ak ich považujete za užitočné, zvážte podporu ich ďalšieho vývoja na Open Collective.", "Podporiť na Open Collective", "Tento zoznam wiki", "prejaví sa po reštarte TiddlyDesktopu", "Uložené. Pre aktiváciu reštartujte TiddlyDesktop.", "odvolať", "Spustiť túto wiki bez sandboxu", "JavaScript tejto wiki pobeží s plným prístupom k vášmu počítaču — môže čítať, meniť a mazať akýkoľvek súbor, ku ktorému máte prístup. Zapnite to len pre wiki, ktorú ste sami napísali alebo ktorej plne dôverujete, a len ak potrebuje spúšťať moduly Node alebo príkazy shellu."],
	"sl-SI": ["Podprite", "TiddlyWiki in TiddlyDesktop sta brezplačna in odprtokodna. Če se vam zdita uporabna, razmislite o podpori njunemu nadaljnjemu razvoju na Open Collective.", "Podprite na Open Collective", "Ta seznam wikijev", "začne veljati po ponovnem zagonu TiddlyDesktopa", "Shranjeno. Za aktivacijo znova zaženite TiddlyDesktop.", "prekliči", "Zaženi ta wiki brez peskovnika", "JavaScript tega wikija se bo izvajal s polnim dostopom do vašega računalnika — lahko bere, spreminja in briše katero koli datoteko, do katere imate dostop. To vklopite le za wiki, ki ste ga napisali sami ali mu popolnoma zaupate, in le če mora izvajati module Node ali ukaze lupine."],
	"mk-MK": ["Поддршка", "TiddlyWiki и TiddlyDesktop се бесплатни и со отворен код. Ако ви се корисни, размислете да го поддржите нивниот натамошен развој на Open Collective.", "Поддржи на Open Collective", "Оваа листа на викија", "стапува на сила по рестартирање на TiddlyDesktop", "Зачувано. Рестартирајте го TiddlyDesktop за да се активира.", "отповикај", "Изврши го ова вики без песочник", "JavaScript-от на ова вики ќе се извршува со целосен пристап до вашиот компјутер — може да чита, менува и брише која било датотека до која вие имате пристап. Вклучете го ова само за вики што сте го напишале сами или на кое целосно му верувате, и само ако мора да извршува Node-модули или shell-команди."],
	"ru-RU": ["Поддержать", "TiddlyWiki и TiddlyDesktop бесплатны и имеют открытый исходный код. Если они вам полезны, поддержите их дальнейшую разработку на Open Collective.", "Поддержать на Open Collective", "Этот список вики", "вступит в силу после перезапуска TiddlyDesktop", "Сохранено. Перезапустите TiddlyDesktop, чтобы активировать.", "отозвать", "Запускать эту вики без песочницы", "Собственный JavaScript этой вики будет выполняться с полным доступом к вашему компьютеру — он сможет читать, изменять и удалять любой файл, доступный вам. Включайте это только для вики, которую вы написали сами или которой полностью доверяете, и только если ей нужно запускать модули Node или команды оболочки."],
	"el-GR": ["Υποστήριξη", "Το TiddlyWiki και το TiddlyDesktop είναι δωρεάν και ανοιχτού κώδικα. Αν σας φαίνονται χρήσιμα, σκεφτείτε να υποστηρίξετε τη συνεχιζόμενη ανάπτυξή τους στο Open Collective.", "Υποστήριξη στο Open Collective", "Αυτή η λίστα wiki", "τίθεται σε ισχύ μετά την επανεκκίνηση του TiddlyDesktop", "Αποθηκεύτηκε. Επανεκκινήστε το TiddlyDesktop για ενεργοποίηση.", "ανάκληση", "Εκτέλεση αυτού του wiki χωρίς sandbox", "Η JavaScript αυτού του wiki θα εκτελείται με πλήρη πρόσβαση στον υπολογιστή σας — μπορεί να διαβάσει, να αλλάξει και να διαγράψει οποιοδήποτε αρχείο μπορείτε κι εσείς. Ενεργοποιήστε το μόνο για ένα wiki που γράψατε εσείς ή εμπιστεύεστε πλήρως, και μόνο αν χρειάζεται να εκτελεί modules Node ή εντολές κελύφους."],
	"he-IL": ["תמיכה", "TiddlyWiki ו-TiddlyDesktop הם חינמיים ובקוד פתוח. אם הם מועילים לך, שקול לתמוך בפיתוח המתמשך שלהם ב-Open Collective.", "תמכו ב-Open Collective", "רשימת ויקי זו", "ייכנס לתוקף לאחר הפעלה מחדש של TiddlyDesktop", "נשמר. הפעל מחדש את TiddlyDesktop כדי להפעיל.", "בטל", "הרץ ויקי זה ללא ארגז חול", "ה-JavaScript של ויקי זה ירוץ עם גישה מלאה למחשב שלך — הוא יוכל לקרוא, לשנות ולמחוק כל קובץ שאתה יכול. הפעל זאת רק עבור ויקי שכתבת בעצמך או שאתה סומך עליו לחלוטין, ורק אם הוא צריך להריץ מודולי Node או פקודות מעטפת."],
	"ar-PS": ["الدعم", "‏TiddlyWiki و TiddlyDesktop مجانيان ومفتوحا المصدر. إذا وجدتهما مفيدين، ففكّر في دعم تطويرهما المستمر على Open Collective.", "ادعم على Open Collective", "قائمة الويكي هذه", "يسري بعد إعادة تشغيل TiddlyDesktop", "تم الحفظ. أعد تشغيل TiddlyDesktop للتفعيل.", "إلغاء", "تشغيل هذا الويكي بدون صندوق العزل", "ستعمل شيفرة JavaScript الخاصة بهذا الويكي بوصول كامل إلى حاسوبك — يمكنها قراءة أي ملف تستطيع الوصول إليه وتعديله وحذفه. لا تفعّل هذا إلا لويكي كتبته بنفسك أو تثق به تمامًا، وفقط إذا كان بحاجة إلى تشغيل وحدات Node أو أوامر الصدفة."],
	"fa-IR": ["پشتیبانی", "TiddlyWiki و TiddlyDesktop رایگان و متن‌باز هستند. اگر برایتان مفیدند، از توسعهٔ مستمر آن‌ها در Open Collective پشتیبانی کنید.", "پشتیبانی در Open Collective", "همین فهرست ویکی", "پس از راه‌اندازی دوبارهٔ TiddlyDesktop اعمال می‌شود", "ذخیره شد. برای فعال‌سازی، TiddlyDesktop را دوباره راه‌اندازی کنید.", "لغو", "اجرای این ویکی بدون جعبهٔ ایمنی", "جاوااسکریپت خودِ این ویکی با دسترسی کامل به رایانهٔ شما اجرا می‌شود — می‌تواند هر پرونده‌ای را که شما می‌توانید بخوانید، تغییر دهد و حذف کند. این را فقط برای ویکی‌ای فعال کنید که خودتان نوشته‌اید یا کاملاً به آن اعتماد دارید، و تنها اگر لازم دارد ماژول‌های Node یا فرمان‌های پوسته را اجرا کند."],
	"hi-IN": ["सहयोग", "TiddlyWiki और TiddlyDesktop नि:शुल्क और ओपन सोर्स हैं। यदि वे आपके लिए उपयोगी हैं, तो Open Collective पर उनके निरंतर विकास में सहयोग देने पर विचार करें।", "Open Collective पर सहयोग दें", "यह विकी सूची", "TiddlyDesktop को पुनः आरंभ करने के बाद प्रभावी होता है", "सहेजा गया। सक्रिय करने के लिए TiddlyDesktop पुनः आरंभ करें।", "रद्द करें", "इस विकी को सैंडबॉक्स के बिना चलाएँ", "इस विकी का अपना JavaScript आपके कंप्यूटर तक पूरी पहुँच के साथ चलेगा — यह हर वह फ़ाइल पढ़, बदल और मिटा सकता है जो आप पढ़ सकते हैं। इसे केवल उसी विकी के लिए सक्षम करें जिसे आपने स्वयं लिखा है या जिस पर आप पूरी तरह भरोसा करते हैं, और तभी जब उसे Node मॉड्यूल या शेल कमांड चलाने की आवश्यकता हो।"],
	"pa-IN": ["ਸਹਿਯੋਗ", "TiddlyWiki ਅਤੇ TiddlyDesktop ਮੁਫ਼ਤ ਅਤੇ ਓਪਨ ਸੋਰਸ ਹਨ। ਜੇ ਇਹ ਤੁਹਾਡੇ ਲਈ ਲਾਭਦਾਇਕ ਹਨ, ਤਾਂ Open Collective ਉੱਤੇ ਇਹਨਾਂ ਦੇ ਨਿਰੰਤਰ ਵਿਕਾਸ ਵਿੱਚ ਸਹਿਯੋਗ ਦੇਣ ਬਾਰੇ ਵਿਚਾਰ ਕਰੋ।", "Open Collective ਉੱਤੇ ਸਹਿਯੋਗ ਦਿਓ", "ਇਹ ਵਿਕੀ ਸੂਚੀ", "TiddlyDesktop ਨੂੰ ਮੁੜ ਚਾਲੂ ਕਰਨ ਤੋਂ ਬਾਅਦ ਲਾਗੂ ਹੁੰਦਾ ਹੈ", "ਸੰਭਾਲਿਆ ਗਿਆ। ਸਰਗਰਮ ਕਰਨ ਲਈ TiddlyDesktop ਮੁੜ ਚਾਲੂ ਕਰੋ।", "ਰੱਦ ਕਰੋ", "ਇਸ ਵਿਕੀ ਨੂੰ ਸੈਂਡਬਾਕਸ ਤੋਂ ਬਿਨਾਂ ਚਲਾਓ", "ਇਸ ਵਿਕੀ ਦਾ ਆਪਣਾ JavaScript ਤੁਹਾਡੇ ਕੰਪਿਊਟਰ ਤੱਕ ਪੂਰੀ ਪਹੁੰਚ ਨਾਲ ਚੱਲੇਗਾ — ਇਹ ਹਰ ਉਹ ਫਾਈਲ ਪੜ੍ਹ, ਬਦਲ ਅਤੇ ਮਿਟਾ ਸਕਦਾ ਹੈ ਜੋ ਤੁਸੀਂ ਪੜ੍ਹ ਸਕਦੇ ਹੋ। ਇਸਨੂੰ ਸਿਰਫ਼ ਉਸ ਵਿਕੀ ਲਈ ਚਾਲੂ ਕਰੋ ਜੋ ਤੁਸੀਂ ਖੁਦ ਲਿਖੀ ਹੈ ਜਾਂ ਜਿਸ ਉੱਤੇ ਤੁਹਾਨੂੰ ਪੂਰਾ ਭਰੋਸਾ ਹੈ, ਅਤੇ ਤਾਂ ਹੀ ਜੇ ਇਸਨੂੰ Node ਮੌਡਿਊਲ ਜਾਂ ਸ਼ੈੱਲ ਕਮਾਂਡਾਂ ਚਲਾਉਣ ਦੀ ਲੋੜ ਹੋਵੇ।"],
	"ia-IA": ["Supportar", "TiddlyWiki e TiddlyDesktop es gratuite e de fonte aperte. Si tu los trova utile, considera supportar lor disveloppamento continue in Open Collective.", "Supportar in Open Collective", "Iste lista de wikis", "prende effecto post le reinitiation de TiddlyDesktop", "Salveguardate. Reinitia TiddlyDesktop pro activar lo.", "revocar", "Exequer iste wiki sin le sandbox", "Le proprie JavaScript de iste wiki se exequera con accesso complete a tu computator — illo pote leger, modificar e deler omne file que tu pote leger. Activa isto solmente pro un wiki que tu ha scribite o in le qual tu confide completemente, e solmente si illo debe exequer modulos Node o commandos de shell."],
	"ja-JP": ["支援", "TiddlyWiki と TiddlyDesktop は無料でオープンソースです。役に立っていると感じたら、Open Collective での継続的な開発支援をご検討ください。", "Open Collective で支援する", "このウィキ一覧", "TiddlyDesktop の再起動後に反映されます", "保存しました。有効にするには TiddlyDesktop を再起動してください。", "取り消す", "このウィキをサンドボックスなしで実行する", "このウィキ自身の JavaScript が、あなたのコンピューターへの完全なアクセス権をもって実行されます — あなたが読めるファイルはすべて読み取り・変更・削除できます。自分で書いたウィキ、または完全に信頼できるウィキで、Node モジュールやシェルコマンドの実行が必要な場合にのみ有効にしてください。"],
	"ko-KR": ["후원", "TiddlyWiki와 TiddlyDesktop은 무료이며 오픈 소스입니다. 유용하다고 느끼신다면 Open Collective에서 지속적인 개발을 후원해 주세요.", "Open Collective에서 후원하기", "이 위키 목록", "TiddlyDesktop을 다시 시작한 뒤에 적용됩니다", "저장했습니다. 활성화하려면 TiddlyDesktop을 다시 시작하세요.", "해제", "이 위키를 샌드박스 없이 실행", "이 위키의 자체 JavaScript가 컴퓨터에 대한 전체 접근 권한으로 실행됩니다 — 사용자가 읽을 수 있는 모든 파일을 읽고, 변경하고, 삭제할 수 있습니다. 직접 작성했거나 완전히 신뢰하는 위키에 한해, 그리고 Node 모듈이나 셸 명령을 실행해야 하는 경우에만 켜세요."],
	"zh-Hans": ["支持", "TiddlyWiki 和 TiddlyDesktop 是免费且开源的。如果你觉得它们有用，欢迎在 Open Collective 上支持它们的持续开发。", "在 Open Collective 上支持", "此 wiki 列表", "重启 TiddlyDesktop 后生效", "已保存。重启 TiddlyDesktop 以启用。", "撤销", "在不使用沙箱的情况下运行此 wiki", "此 wiki 自身的 JavaScript 将以对你计算机的完全访问权限运行——它可以读取、修改和删除任何你能读取的文件。仅对你自己编写或完全信任的 wiki 启用，并且仅在它需要运行 Node 模块或 shell 命令时启用。"],
	"zh-Hant": ["支持", "TiddlyWiki 和 TiddlyDesktop 是免費且開源的。如果你覺得它們有用，歡迎在 Open Collective 上支持它們的持續開發。", "在 Open Collective 上支持", "此 wiki 清單", "重新啟動 TiddlyDesktop 後生效", "已儲存。重新啟動 TiddlyDesktop 以啟用。", "撤銷", "在不使用沙箱的情況下執行此 wiki", "此 wiki 自身的 JavaScript 將以對你電腦的完整存取權限執行——它可以讀取、修改和刪除任何你能讀取的檔案。僅對你自己撰寫或完全信任的 wiki 啟用，且僅在它需要執行 Node 模組或 shell 命令時啟用。"]
};
Object.keys(LATER_STRINGS).forEach(function(lang) {
	if(!T[lang]) { return; }
	LATER_KEYS.forEach(function(k, i) { T[lang][k] = LATER_STRINGS[lang][i]; });
});

// ── Settings panel chrome and the convert dialogs ───────────────────────────
// Captions, button and checkbox labels, and the confirm/alert text the convert flow shows. The
// long explanatory prose in those panels is deliberately NOT here: it stays English rather than
// ship an unreviewed machine translation of, among other things, the unsandboxed warning.
var SETTINGS_KEYS = ["Settings/Backups", "Settings/Flags", "Settings/Spellcheck", "Settings/Plugins", "Settings/Templates", "Settings/UserConfig", "Settings/WikiListTitle", "Settings/TrustedPaths", "Settings/Version", "Settings/ResetToDefault", "Settings/OpenChromiumFlags", "Settings/OpenConfigFolder", "Settings/InstallPlugins", "Spellcheck/Enable", "Spellcheck/LanguageHeading", "Spellcheck/GoogleHeading", "Spellcheck/EnableGoogle", "Spellcheck/DiagnosticsHeading", "Spellcheck/Refresh", "TrustedPaths/RevokeTooltip", "Convert/NoDestination", "Convert/ToFile", "Convert/ToFolder", "Convert/OriginalKept", "Convert/InProgress", "Convert/Failed", "Convert/Done"];
var SETTINGS_STRINGS = {
	"de-DE": ["Sicherungen", "Chromium-Flags", "Rechtschreibprüfung", "Plugins", "Vorlagen", "Benutzerkonfigurationsordner", "Titel des Wiki-Listen-Fensters", "Vertraute Pfade", "Version", "Auf Standard zurücksetzen", "Chromium-Flags öffnen", "Konfigurationsordner öffnen", "Plugins installieren…", "Lokale Rechtschreibprüfung aktivieren", "Sprache der Rechtschreibprüfung", "Datenschutz: Googles Rechtschreibdienst", "Googles Rechtschreibdienst verwenden (sendet deinen Text an Google)", "Diagnose", "Aktualisieren", "Diesem Wiki den Zugriff auf diesen Pfad entziehen", "Für die Umwandlung wurde kein Ziel gewählt.", "Dieses Wiki von einem Ordner in eine einzelne Datei umwandeln?", "Dieses Wiki von einer einzelnen Datei in einen Ordner umwandeln?", "Das ursprüngliche Wiki wird nicht gelöscht.", "Wiki wird umgewandelt…", "Umwandlung fehlgeschlagen:", "Wiki umgewandelt:"],
	"fr-FR": ["Sauvegardes", "Indicateurs Chromium", "Correcteur orthographique", "Plugins", "Modèles", "Dossier de configuration utilisateur", "Titre de la fenêtre de la liste des wikis", "Chemins de confiance", "Version", "Réinitialiser", "Ouvrir les indicateurs Chromium", "Ouvrir le dossier de configuration", "Installer des plugins…", "Activer le correcteur orthographique local", "Langue du correcteur orthographique", "Confidentialité : service de correction Google", "Utiliser le service de correction distant de Google (envoie votre texte à Google)", "Diagnostics", "Actualiser", "Empêcher ce wiki de lire ce chemin", "Aucune destination n'a été choisie pour la conversion.", "Convertir ce wiki d'un dossier en un fichier unique ?", "Convertir ce wiki d'un fichier unique en un dossier ?", "Le wiki d'origine n'est pas supprimé.", "Conversion du wiki…", "Échec de la conversion :", "Wiki converti :"],
	"es-ES": ["Copias de seguridad", "Indicadores de Chromium", "Corrector ortográfico", "Plugins", "Plantillas", "Carpeta de configuración del usuario", "Título de la ventana de la lista de wikis", "Rutas de confianza", "Versión", "Restablecer", "Abrir los indicadores de Chromium", "Abrir la carpeta de configuración", "Instalar plugins…", "Activar el corrector ortográfico local", "Idioma del corrector ortográfico", "Privacidad: servicio de corrección de Google", "Usar el servicio de corrección remoto de Google (envía tu texto a Google)", "Diagnóstico", "Actualizar", "Impedir que este wiki lea esta ruta", "No se eligió ningún destino para la conversión.", "¿Convertir este wiki de una carpeta a un solo archivo?", "¿Convertir este wiki de un solo archivo a una carpeta?", "El wiki original no se elimina.", "Convirtiendo el wiki…", "Error en la conversión:", "Wiki convertido:"],
	"ca-ES": ["Còpies de seguretat", "Indicadors de Chromium", "Corrector ortogràfic", "Connectors", "Plantilles", "Carpeta de configuració de l'usuari", "Títol de la finestra de la llista de wikis", "Camins de confiança", "Versió", "Restableix", "Obre els indicadors de Chromium", "Obre la carpeta de configuració", "Instal·la connectors…", "Activa el corrector ortogràfic local", "Idioma del corrector ortogràfic", "Privadesa: servei de correcció de Google", "Fes servir el servei de correcció remot de Google (envia el teu text a Google)", "Diagnòstic", "Actualitza", "Impedeix que aquest wiki llegeixi aquest camí", "No s'ha triat cap destinació per a la conversió.", "Voleu convertir aquest wiki d'una carpeta a un sol fitxer?", "Voleu convertir aquest wiki d'un sol fitxer a una carpeta?", "El wiki original no s'elimina.", "S'està convertint el wiki…", "La conversió ha fallat:", "Wiki convertit:"],
	"it-IT": ["Backup", "Flag di Chromium", "Controllo ortografico", "Plugin", "Modelli", "Cartella di configurazione utente", "Titolo della finestra dell'elenco wiki", "Percorsi attendibili", "Versione", "Ripristina i valori predefiniti", "Apri i flag di Chromium", "Apri la cartella di configurazione", "Installa plugin…", "Attiva il controllo ortografico locale", "Lingua del controllo ortografico", "Privacy: servizio di correzione Google", "Usa il servizio di correzione remoto di Google (invia il tuo testo a Google)", "Diagnostica", "Aggiorna", "Impedisci a questo wiki di leggere questo percorso", "Non è stata scelta alcuna destinazione per la conversione.", "Convertire questo wiki da cartella a file singolo?", "Convertire questo wiki da file singolo a cartella?", "Il wiki originale non viene eliminato.", "Conversione del wiki…", "Conversione non riuscita:", "Wiki convertito:"],
	"pt-BR": ["Backups", "Flags do Chromium", "Verificação ortográfica", "Plugins", "Modelos", "Pasta de configuração do usuário", "Título da janela da lista de wikis", "Caminhos confiáveis", "Versão", "Restaurar o padrão", "Abrir as flags do Chromium", "Abrir a pasta de configuração", "Instalar plugins…", "Ativar a verificação ortográfica local", "Idioma da verificação ortográfica", "Privacidade: serviço de correção do Google", "Usar o serviço de correção remoto do Google (envia seu texto ao Google)", "Diagnóstico", "Atualizar", "Impedir que este wiki leia este caminho", "Nenhum destino foi escolhido para a conversão.", "Converter este wiki de uma pasta para um único arquivo?", "Converter este wiki de um único arquivo para uma pasta?", "O wiki original não é excluído.", "Convertendo o wiki…", "Falha na conversão:", "Wiki convertido:"],
	"pt-PT": ["Cópias de segurança", "Flags do Chromium", "Verificação ortográfica", "Plugins", "Modelos", "Pasta de configuração do utilizador", "Título da janela da lista de wikis", "Caminhos fidedignos", "Versão", "Repor a predefinição", "Abrir as flags do Chromium", "Abrir a pasta de configuração", "Instalar plugins…", "Ativar a verificação ortográfica local", "Idioma da verificação ortográfica", "Privacidade: serviço de correção da Google", "Utilizar o serviço de correção remoto da Google (envia o seu texto para a Google)", "Diagnóstico", "Atualizar", "Impedir que este wiki leia este caminho", "Não foi escolhido nenhum destino para a conversão.", "Converter este wiki de uma pasta para um único ficheiro?", "Converter este wiki de um único ficheiro para uma pasta?", "O wiki original não é eliminado.", "A converter o wiki…", "A conversão falhou:", "Wiki convertido:"],
	"nl-NL": ["Back-ups", "Chromium-vlaggen", "Spellingcontrole", "Plug-ins", "Sjablonen", "Map met gebruikersconfiguratie", "Titel van het wikilijstvenster", "Vertrouwde paden", "Versie", "Terugzetten naar standaard", "Chromium-vlaggen openen", "Configuratiemap openen", "Plug-ins installeren…", "Lokale spellingcontrole inschakelen", "Taal van de spellingcontrole", "Privacy: spellingdienst van Google", "De spellingdienst van Google gebruiken (stuurt je tekst naar Google)", "Diagnostiek", "Vernieuwen", "Deze wiki dit pad niet meer laten lezen", "Er is geen bestemming gekozen voor de conversie.", "Deze wiki omzetten van een map naar één bestand?", "Deze wiki omzetten van één bestand naar een map?", "De oorspronkelijke wiki wordt niet verwijderd.", "Wiki wordt omgezet…", "Conversie mislukt:", "Wiki omgezet:"],
	"da-DK": ["Sikkerhedskopier", "Chromium-flag", "Stavekontrol", "Plugins", "Skabeloner", "Mappe med brugerkonfiguration", "Titel på wikiliste-vinduet", "Betroede stier", "Version", "Nulstil til standard", "Åbn Chromium-flag", "Åbn konfigurationsmappen", "Installér plugins…", "Aktivér lokal stavekontrol", "Sprog for stavekontrol", "Privatliv: Googles stavetjeneste", "Brug Googles fjernstavetjeneste (sender din tekst til Google)", "Diagnostik", "Opdater", "Stop denne wiki i at læse denne sti", "Der blev ikke valgt en destination til konverteringen.", "Konvertér denne wiki fra en mappe til én fil?", "Konvertér denne wiki fra én fil til en mappe?", "Den oprindelige wiki slettes ikke.", "Konverterer wiki…", "Konverteringen mislykkedes:", "Wiki konverteret:"],
	"sv-SE": ["Säkerhetskopior", "Chromium-flaggor", "Stavningskontroll", "Tillägg", "Mallar", "Mapp för användarkonfiguration", "Titel på wikilistfönstret", "Betrodda sökvägar", "Version", "Återställ till standard", "Öppna Chromium-flaggor", "Öppna konfigurationsmappen", "Installera tillägg…", "Aktivera lokal stavningskontroll", "Språk för stavningskontroll", "Integritet: Googles stavningstjänst", "Använd Googles fjärrstavningstjänst (skickar din text till Google)", "Diagnostik", "Uppdatera", "Hindra den här wikin från att läsa den här sökvägen", "Ingen destination valdes för konverteringen.", "Konvertera den här wikin från en mapp till en enda fil?", "Konvertera den här wikin från en enda fil till en mapp?", "Den ursprungliga wikin tas inte bort.", "Konverterar wiki…", "Konverteringen misslyckades:", "Wiki konverterad:"],
	"pl-PL": ["Kopie zapasowe", "Flagi Chromium", "Sprawdzanie pisowni", "Wtyczki", "Szablony", "Folder konfiguracji użytkownika", "Tytuł okna listy wiki", "Zaufane ścieżki", "Wersja", "Przywróć domyślne", "Otwórz flagi Chromium", "Otwórz folder konfiguracji", "Zainstaluj wtyczki…", "Włącz lokalne sprawdzanie pisowni", "Język sprawdzania pisowni", "Prywatność: usługa pisowni Google", "Używaj zdalnej usługi pisowni Google (wysyła Twój tekst do Google)", "Diagnostyka", "Odśwież", "Odbierz temu wiki dostęp do tej ścieżki", "Nie wybrano miejsca docelowego konwersji.", "Przekształcić to wiki z folderu w pojedynczy plik?", "Przekształcić to wiki z pojedynczego pliku w folder?", "Oryginalne wiki nie zostanie usunięte.", "Trwa przekształcanie wiki…", "Konwersja nie powiodła się:", "Wiki przekształcone:"],
	"cs-CZ": ["Zálohy", "Příznaky Chromia", "Kontrola pravopisu", "Zásuvné moduly", "Šablony", "Složka uživatelské konfigurace", "Titulek okna se seznamem wiki", "Důvěryhodné cesty", "Verze", "Obnovit výchozí", "Otevřít příznaky Chromia", "Otevřít složku konfigurace", "Nainstalovat zásuvné moduly…", "Zapnout místní kontrolu pravopisu", "Jazyk kontroly pravopisu", "Soukromí: služba pravopisu Google", "Používat vzdálenou službu pravopisu Google (odesílá váš text Googlu)", "Diagnostika", "Obnovit", "Zakázat této wiki číst tuto cestu", "Pro převod nebylo zvoleno žádné umístění.", "Převést tuto wiki ze složky na jediný soubor?", "Převést tuto wiki z jediného souboru na složku?", "Původní wiki se nesmaže.", "Převádím wiki…", "Převod se nezdařil:", "Wiki převedena:"],
	"sk-SK": ["Zálohy", "Príznaky Chromia", "Kontrola pravopisu", "Zásuvné moduly", "Šablóny", "Priečinok používateľskej konfigurácie", "Titulok okna so zoznamom wiki", "Dôveryhodné cesty", "Verzia", "Obnoviť predvolené", "Otvoriť príznaky Chromia", "Otvoriť priečinok konfigurácie", "Nainštalovať zásuvné moduly…", "Zapnúť miestnu kontrolu pravopisu", "Jazyk kontroly pravopisu", "Súkromie: služba pravopisu Google", "Používať vzdialenú službu pravopisu Google (odosiela váš text Googlu)", "Diagnostika", "Obnoviť", "Zakázať tejto wiki čítať túto cestu", "Pre prevod nebolo zvolené žiadne umiestnenie.", "Previesť túto wiki z priečinka na jediný súbor?", "Previesť túto wiki z jediného súboru na priečinok?", "Pôvodná wiki sa nezmaže.", "Prevádzam wiki…", "Prevod zlyhal:", "Wiki prevedená:"],
	"sl-SI": ["Varnostne kopije", "Zastavice Chromiuma", "Preverjanje črkovanja", "Vtičniki", "Predloge", "Mapa uporabniške konfiguracije", "Naslov okna s seznamom wikijev", "Zaupanja vredne poti", "Različica", "Ponastavi na privzeto", "Odpri zastavice Chromiuma", "Odpri mapo konfiguracije", "Namesti vtičnike…", "Vklopi lokalno preverjanje črkovanja", "Jezik preverjanja črkovanja", "Zasebnost: Googlova storitev črkovanja", "Uporabi Googlovo oddaljeno storitev črkovanja (vaše besedilo pošlje Googlu)", "Diagnostika", "Osveži", "Prepreči temu wikiju branje te poti", "Za pretvorbo ni bil izbran cilj.", "Ali naj se ta wiki pretvori iz mape v eno samo datoteko?", "Ali naj se ta wiki pretvori iz ene same datoteke v mapo?", "Izvirni wiki se ne izbriše.", "Pretvarjanje wikija…", "Pretvorba ni uspela:", "Wiki pretvorjen:"],
	"mk-MK": ["Резервни копии", "Знаменца на Chromium", "Проверка на правопис", "Приклучоци", "Шаблони", "Папка со корисничка конфигурација", "Наслов на прозорецот со листа на викија", "Доверливи патеки", "Верзија", "Врати на стандардно", "Отвори ги знаменцата на Chromium", "Отвори ја папката со конфигурација", "Инсталирај приклучоци…", "Вклучи локална проверка на правопис", "Јазик на проверката на правопис", "Приватност: услугата за правопис на Google", "Користи ја оддалечената услуга за правопис на Google (го испраќа твојот текст до Google)", "Дијагностика", "Освежи", "Забрани му на ова вики да ја чита оваа патека", "Не е избрана дестинација за претворањето.", "Да се претвори ова вики од папка во една датотека?", "Да се претвори ова вики од една датотека во папка?", "Изворното вики не се брише.", "Викито се претвора…", "Претворањето не успеа:", "Викито е претворено:"],
	"ru-RU": ["Резервные копии", "Флаги Chromium", "Проверка орфографии", "Плагины", "Шаблоны", "Папка пользовательской конфигурации", "Заголовок окна списка вики", "Доверенные пути", "Версия", "Сбросить к значению по умолчанию", "Открыть флаги Chromium", "Открыть папку конфигурации", "Установить плагины…", "Включить локальную проверку орфографии", "Язык проверки орфографии", "Приватность: служба орфографии Google", "Использовать удалённую службу орфографии Google (отправляет ваш текст в Google)", "Диагностика", "Обновить", "Запретить этой вики читать этот путь", "Для преобразования не выбрано место назначения.", "Преобразовать эту вики из папки в один файл?", "Преобразовать эту вики из одного файла в папку?", "Исходная вики не удаляется.", "Преобразование вики…", "Не удалось выполнить преобразование:", "Вики преобразована:"],
	"el-GR": ["Αντίγραφα ασφαλείας", "Σημαίες Chromium", "Ορθογραφικός έλεγχος", "Πρόσθετα", "Πρότυπα", "Φάκελος ρυθμίσεων χρήστη", "Τίτλος παραθύρου λίστας wiki", "Έμπιστες διαδρομές", "Έκδοση", "Επαναφορά στις προεπιλογές", "Άνοιγμα σημαιών Chromium", "Άνοιγμα φακέλου ρυθμίσεων", "Εγκατάσταση προσθέτων…", "Ενεργοποίηση τοπικού ορθογραφικού ελέγχου", "Γλώσσα ορθογραφικού ελέγχου", "Απόρρητο: υπηρεσία ορθογραφίας της Google", "Χρήση της απομακρυσμένης υπηρεσίας ορθογραφίας της Google (στέλνει το κείμενό σας στην Google)", "Διαγνωστικά", "Ανανέωση", "Να μην μπορεί αυτό το wiki να διαβάζει αυτή τη διαδρομή", "Δεν επιλέχθηκε προορισμός για τη μετατροπή.", "Μετατροπή αυτού του wiki από φάκελο σε ένα μόνο αρχείο;", "Μετατροπή αυτού του wiki από ένα μόνο αρχείο σε φάκελο;", "Το αρχικό wiki δεν διαγράφεται.", "Μετατροπή wiki…", "Η μετατροπή απέτυχε:", "Το wiki μετατράπηκε:"],
	"he-IL": ["גיבויים", "דגלי Chromium", "בדיקת איות", "תוספים", "תבניות", "תיקיית תצורת המשתמש", "כותרת חלון רשימת הוויקי", "נתיבים מהימנים", "גרסה", "אפס לברירת המחדל", "פתח את דגלי Chromium", "פתח את תיקיית התצורה", "התקן תוספים…", "הפעל בדיקת איות מקומית", "שפת בדיקת האיות", "פרטיות: שירות האיות של Google", "השתמש בשירות האיות המרוחק של Google (שולח את הטקסט שלך ל-Google)", "אבחון", "רענן", "מנע מוויקי זה לקרוא נתיב זה", "לא נבחר יעד להמרה.", "להמיר ויקי זה מתיקייה לקובץ יחיד?", "להמיר ויקי זה מקובץ יחיד לתיקייה?", "הוויקי המקורי אינו נמחק.", "ממיר ויקי…", "ההמרה נכשלה:", "הוויקי הומר:"],
	"ar-PS": ["النسخ الاحتياطية", "رايات Chromium", "التدقيق الإملائي", "الإضافات", "القوالب", "مجلد إعدادات المستخدم", "عنوان نافذة قائمة الويكي", "المسارات الموثوقة", "الإصدار", "إعادة التعيين إلى الافتراضي", "فتح رايات Chromium", "فتح مجلد الإعدادات", "تثبيت الإضافات…", "تفعيل التدقيق الإملائي المحلي", "لغة التدقيق الإملائي", "الخصوصية: خدمة التدقيق الإملائي من Google", "استخدام خدمة التدقيق الإملائي البعيدة من Google (ترسل نصك إلى Google)", "التشخيص", "تحديث", "منع هذا الويكي من قراءة هذا المسار", "لم يتم اختيار وجهة للتحويل.", "هل تريد تحويل هذا الويكي من مجلد إلى ملف واحد؟", "هل تريد تحويل هذا الويكي من ملف واحد إلى مجلد؟", "لا يُحذف الويكي الأصلي.", "جارٍ تحويل الويكي…", "فشل التحويل:", "تم تحويل الويكي:"],
	"fa-IR": ["نسخه‌های پشتیبان", "پرچم‌های Chromium", "غلط‌یاب املایی", "افزونه‌ها", "قالب‌ها", "پوشهٔ پیکربندی کاربر", "عنوان پنجرهٔ فهرست ویکی", "مسیرهای مورد اعتماد", "نسخه", "بازنشانی به پیش‌فرض", "گشودن پرچم‌های Chromium", "گشودن پوشهٔ پیکربندی", "نصب افزونه‌ها…", "فعال‌سازی غلط‌یاب املایی محلی", "زبان غلط‌یاب املایی", "حریم خصوصی: سرویس املایی Google", "استفاده از سرویس املایی راه‌دور Google (متن شما را به Google می‌فرستد)", "عیب‌یابی", "تازه‌سازی", "جلوگیری از خواندن این مسیر توسط این ویکی", "برای تبدیل، مقصدی انتخاب نشد.", "این ویکی از یک پوشه به یک پروندهٔ واحد تبدیل شود؟", "این ویکی از یک پروندهٔ واحد به یک پوشه تبدیل شود؟", "ویکی اصلی حذف نمی‌شود.", "در حال تبدیل ویکی…", "تبدیل ناموفق بود:", "ویکی تبدیل شد:"],
	"hi-IN": ["बैकअप", "Chromium फ़्लैग", "वर्तनी जाँच", "प्लगइन", "टेम्पलेट", "उपयोगकर्ता कॉन्फ़िगरेशन फ़ोल्डर", "विकी सूची विंडो का शीर्षक", "विश्वसनीय पथ", "संस्करण", "डिफ़ॉल्ट पर रीसेट करें", "Chromium फ़्लैग खोलें", "कॉन्फ़िगरेशन फ़ोल्डर खोलें", "प्लगइन इंस्टॉल करें…", "स्थानीय वर्तनी जाँच सक्षम करें", "वर्तनी जाँच की भाषा", "गोपनीयता: Google की वर्तनी सेवा", "Google की दूरस्थ वर्तनी सेवा का उपयोग करें (आपका पाठ Google को भेजता है)", "निदान", "ताज़ा करें", "इस विकी को यह पथ पढ़ने से रोकें", "रूपांतरण के लिए कोई गंतव्य नहीं चुना गया।", "इस विकी को फ़ोल्डर से एकल फ़ाइल में बदलें?", "इस विकी को एकल फ़ाइल से फ़ोल्डर में बदलें?", "मूल विकी हटाई नहीं जाती।", "विकी बदली जा रही है…", "रूपांतरण विफल:", "विकी बदली गई:"],
	"pa-IN": ["ਬੈਕਅੱਪ", "Chromium ਫਲੈਗ", "ਸ਼ਬਦ-ਜੋੜ ਜਾਂਚ", "ਪਲੱਗਇਨ", "ਟੈਂਪਲੇਟ", "ਵਰਤੋਂਕਾਰ ਸੰਰਚਨਾ ਫੋਲਡਰ", "ਵਿਕੀ ਸੂਚੀ ਵਿੰਡੋ ਦਾ ਸਿਰਲੇਖ", "ਭਰੋਸੇਯੋਗ ਮਾਰਗ", "ਵਰਜਨ", "ਮੂਲ ਉੱਤੇ ਮੁੜ-ਸੈੱਟ ਕਰੋ", "Chromium ਫਲੈਗ ਖੋਲ੍ਹੋ", "ਸੰਰਚਨਾ ਫੋਲਡਰ ਖੋਲ੍ਹੋ", "ਪਲੱਗਇਨ ਇੰਸਟਾਲ ਕਰੋ…", "ਸਥਾਨਕ ਸ਼ਬਦ-ਜੋੜ ਜਾਂਚ ਚਾਲੂ ਕਰੋ", "ਸ਼ਬਦ-ਜੋੜ ਜਾਂਚ ਦੀ ਭਾਸ਼ਾ", "ਨਿੱਜਤਾ: Google ਦੀ ਸ਼ਬਦ-ਜੋੜ ਸੇਵਾ", "Google ਦੀ ਦੂਰ-ਦੁਰਾਡੇ ਸ਼ਬਦ-ਜੋੜ ਸੇਵਾ ਵਰਤੋ (ਤੁਹਾਡਾ ਪਾਠ Google ਨੂੰ ਭੇਜਦੀ ਹੈ)", "ਨਿਦਾਨ", "ਤਾਜ਼ਾ ਕਰੋ", "ਇਸ ਵਿਕੀ ਨੂੰ ਇਹ ਮਾਰਗ ਪੜ੍ਹਨ ਤੋਂ ਰੋਕੋ", "ਤਬਦੀਲੀ ਲਈ ਕੋਈ ਟਿਕਾਣਾ ਨਹੀਂ ਚੁਣਿਆ ਗਿਆ।", "ਕੀ ਇਸ ਵਿਕੀ ਨੂੰ ਫੋਲਡਰ ਤੋਂ ਇੱਕ ਫਾਈਲ ਵਿੱਚ ਬਦਲਣਾ ਹੈ?", "ਕੀ ਇਸ ਵਿਕੀ ਨੂੰ ਇੱਕ ਫਾਈਲ ਤੋਂ ਫੋਲਡਰ ਵਿੱਚ ਬਦਲਣਾ ਹੈ?", "ਅਸਲ ਵਿਕੀ ਮਿਟਾਈ ਨਹੀਂ ਜਾਂਦੀ।", "ਵਿਕੀ ਬਦਲੀ ਜਾ ਰਹੀ ਹੈ…", "ਤਬਦੀਲੀ ਅਸਫਲ:", "ਵਿਕੀ ਬਦਲੀ ਗਈ:"],
	"ia-IA": ["Copias de reserva", "Marcatores de Chromium", "Correction orthographic", "Plugins", "Modellos", "Dossier de configuration del usator", "Titulo del fenestra del lista de wikis", "Camminos fidate", "Version", "Reinitialisar al predefinition", "Aperir le marcatores de Chromium", "Aperir le dossier de configuration", "Installar plugins…", "Activar le correction orthographic local", "Lingua del correction orthographic", "Confidentialitate: le servicio orthographic de Google", "Usar le servicio orthographic remote de Google (invia tu texto a Google)", "Diagnostica", "Actualisar", "Impedir que iste wiki lege iste cammino", "Nulle destination esseva seligite pro le conversion.", "Converter iste wiki de un dossier a un sol file?", "Converter iste wiki de un sol file a un dossier?", "Le wiki original non es delite.", "Convertente le wiki…", "Le conversion falleva:", "Wiki convertite:"],
	"ja-JP": ["バックアップ", "Chromium フラグ", "スペルチェック", "プラグイン", "テンプレート", "ユーザー設定フォルダー", "ウィキ一覧ウィンドウのタイトル", "信頼済みパス", "バージョン", "既定値に戻す", "Chromium フラグを開く", "設定フォルダーを開く", "プラグインをインストール…", "ローカルのスペルチェックを有効にする", "スペルチェックの言語", "プライバシー: Google のスペルチェックサービス", "Google のリモートスペルチェックサービスを使う（入力内容が Google に送信されます）", "診断", "更新", "このウィキがこのパスを読めないようにする", "変換先が選択されていません。", "このウィキをフォルダーから単一ファイルに変換しますか？", "このウィキを単一ファイルからフォルダーに変換しますか？", "元のウィキは削除されません。", "ウィキを変換しています…", "変換に失敗しました:", "ウィキを変換しました:"],
	"ko-KR": ["백업", "Chromium 플래그", "맞춤법 검사", "플러그인", "템플릿", "사용자 설정 폴더", "위키 목록 창 제목", "신뢰된 경로", "버전", "기본값으로 되돌리기", "Chromium 플래그 열기", "설정 폴더 열기", "플러그인 설치…", "로컬 맞춤법 검사 사용", "맞춤법 검사 언어", "개인정보: Google 맞춤법 서비스", "Google의 원격 맞춤법 서비스 사용(입력한 텍스트가 Google로 전송됩니다)", "진단", "새로 고침", "이 위키가 이 경로를 읽지 못하게 하기", "변환할 대상 위치를 선택하지 않았습니다.", "이 위키를 폴더에서 단일 파일로 변환할까요?", "이 위키를 단일 파일에서 폴더로 변환할까요?", "원본 위키는 삭제되지 않습니다.", "위키를 변환하는 중…", "변환 실패:", "위키 변환 완료:"],
	"zh-Hans": ["备份", "Chromium 标志", "拼写检查", "插件", "模板", "用户配置文件夹", "wiki 列表窗口标题", "受信任路径", "版本", "重置为默认值", "打开 Chromium 标志", "打开配置文件夹", "安装插件…", "启用本地拼写检查", "拼写检查语言", "隐私：Google 拼写检查服务", "使用 Google 的远程拼写检查服务（会把你输入的文本发送给 Google）", "诊断", "刷新", "禁止此 wiki 读取该路径", "未为转换选择目标位置。", "要把这个 wiki 从文件夹转换为单个文件吗？", "要把这个 wiki 从单个文件转换为文件夹吗？", "原始 wiki 不会被删除。", "正在转换 wiki…", "转换失败：", "已转换 wiki："],
	"zh-Hant": ["備份", "Chromium 旗標", "拼字檢查", "外掛", "範本", "使用者設定資料夾", "wiki 清單視窗標題", "受信任路徑", "版本", "重設為預設值", "開啟 Chromium 旗標", "開啟設定資料夾", "安裝外掛…", "啟用本機拼字檢查", "拼字檢查語言", "隱私：Google 拼字檢查服務", "使用 Google 的遠端拼字檢查服務（會把你輸入的文字傳送給 Google）", "診斷", "重新整理", "禁止此 wiki 讀取該路徑", "尚未為轉換選擇目標位置。", "要把這個 wiki 從資料夾轉換為單一檔案嗎？", "要把這個 wiki 從單一檔案轉換為資料夾嗎？", "原始 wiki 不會被刪除。", "正在轉換 wiki…", "轉換失敗：", "已轉換 wiki："]
};
Object.keys(SETTINGS_STRINGS).forEach(function(lang) {
	if(!T[lang]) { return; }
	SETTINGS_KEYS.forEach(function(k, i) { T[lang][k] = SETTINGS_STRINGS[lang][i]; });
});
