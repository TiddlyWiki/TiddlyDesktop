 ...
 * native `TDCollab` @JavascriptInterface (see collab/CollabBridge.kt and README.md →
 * "The `window._nwjs*` bridge contract"). Injected in WikiActivity.onPageFinished().
 *
 * All four bridges are implemented: A (HTTP + openExternal), B (relay WebSocket),
 * C (wiki directory + asset file I/O), D (LAN peers, via a native Node helper). D is
 * probed for at runtime, so an older app build simply stays relay-only.
 */
(function () {
	...
	// ── C) Wiki directory + asset file I/O ──────────────────────────────────────
